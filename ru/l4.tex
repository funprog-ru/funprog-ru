...
теории множеств в роли основания.  Есть интересные связи между теорией
типов и программированием (Мартин-Лёф и другие).
...
\heading{Программное обоснование типов}
...
В то же время некоторые программисты находят их как раздражающее
ограничение.  Как нам достичь оптимального баланса?
...
типов, например {\red \tt int} и {\red \tt bool}, и мы можем составлять
новые типы из них, используя конструктор функциональных типов,
например
...
Тут термы такие же, как были раньше; и есть отдельное понятие
...
Описанная выше система продолжает работать, если мы добавим ещё
...
Это просто отлично~--- любая программа завершается.  Однако, это
...
Мы не можем задавать произвольные рекурсивные функции, иначе
мы смогли бы задать функцию, которая никогда не завершается.
...
не полностью типизирована, так как она применяет {\red $x$} к самому себе.
%Clearly {\red $Y = \lamb{f} (\lamb{x} f(x\; x)) (\lamb{x} f(x\; x))$} isn't
%well-typed because it applies {\red $x$} to itself.

Таким образом, нам надо найти способ задания произвольных рекурсивных
функций, которые полностью типизированы.
%So we simply add a way of defining arbitrary recursive functions that {\em is}
%well-typed.

Введём семейство полиморфических операторов рекурсии в виде
