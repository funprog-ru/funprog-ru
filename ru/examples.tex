...
Как мы говорили, ML изначально был спроектирован как метаязык для автоматизированного 
доказательства теорем. Однако он пригоден и для многих других приложений, по большей
части из области  <<символьных вычислений>>. В этой главе мы дадим несколько характерных 
примеров использования ML. От читателя не требуется досконального
знания всех деталей, как, например, в случае приближения вещественных
чисел, приведённом далее. Однако важно позапускать эти программы, выполнить 
упражнения. Поскольку нет лучшего способа почувствовать, как  ML используется 
в действительности.
...
Фраза <<символьное вычисление>> довольно неопределённа, но грубо говоря, она 
охватывает приложения занимающиеся не просто численными расчётами, а манипуляциями 
над математическими {\em выражениями}, в общем случае содержащими переменные. Есть 
несколько успешных <<систем компьютерной алгебры>> таких как 
Maxima, Maple и Mathematica. Они могут выполнять полезные
математические операции: разложение многочленов на множители, 
дифференцирование и интегрирование выражений и т.д. Они также пригодны и для
обычных вычислений. Мы покажем, как задачи такого рода могут быть решены с помощью ML.
...
Мы выбрали символьное дифференцирование для нашего примера, потому что для него
есть довольно простой и известный алгоритм. Читатель, возможно, знаком
...
функций. Поскольку каждый может систематически использовать  эти правила
для нахождения производных, то возможно и запрограммировать их на ML. 
...
Мы будем допускать математические выражения в весьма общей форме. Они могут
быть построены из переменных и констант, с использованием операторов. Операторы
могут быть унарными, бинарными, тернарными, или, в общем случае, арности~$n$.
Это выражается с помощью следующего рекурсивного типа данных:
...
для всех систем символьных вычислений, и обычно интерфейс системы содержит
{\em синтаксический анализатор (парсер)} и {\em систему ввода-вывода},
позволяющую вводить и выводить данные в читаемой форме. Отложим пока подробное
обсуждение синтаксического разбора, поскольку эта, сама по себе важная тема, будет обсуждаться 
далее, а сейчас мы напишем простенькую систему вывода для наших выражений. 
Чтобы, по крайней мере, можно было видеть что происходит. Мы хотим, чтобы 
поддерживались обычные соглашения, как-то:
...
$f(x_1,\ldots,x_n)$, т.е. записывается имя функции и рядом, в скобках
список аргументов.
...
\item Инфиксные бинарные функции, такие как $+$ записываются между
своими аргументами.
...
Сначала мы объявим список инфиксных операторов~-- список пар из имени оператора
и его приоритета.
...
стандартной практикой. Они обычно называются {\em ассоциативными списками} 
и весьма распространены в функциональном программировании.\footnote{Для более
тяжеловесных приложений больше подойдут альтернативы, такие как хеш-таблицы,
...
частичную функцию мы используем {\tt assoc}:
...
Обратите внимание, если мы когда - либо изменим этот список операторов, 
то всякую функцию, вроде {\tt get\_precedence}, использующую его, потребуется 
переопределить. Это является основной причиной, по которой многие  LISP программисты 
предпочитают динамическое связывание. Множество инфиксных операторов можно сделать  
произвольно расширяемым, используя обращение по ссылке:
...
Отметим, что по правилам вычисления в ML, разыменование указателя применяется
только после того, как {\tt get\_precedence} получает свои аргументы, и, следовательно, 
результаты во множестве операторов. Мы также можем задать функцию, определяющую,
является ли некоторая функция инфиксным оператором. Можно просто использовать 
{\tt get\_precedence} и смотреть сработала ли она:
...
Альтернатива заключается в использовании общей функции {\tt can}, которая проверяет 
успешность вычисления другой функции:
...
\noindent Мы будем использовать следующие функции из уже рассмотренных:
...
\noindent Без дальнейших хлопот, вот функция для конвертации терма в строку.
...
Первый аргумент {\tt prec} обозначает уровень приоритета оператора, для которого
текущее рассматриваемое выражение является непосредственным подтермом. 
Так что если текущее выражение имеет инфиксный оператор с некоторым приоритетом, то 
скобки необходимы, если приоритет этого оператора ниже {\tt prec}. Например,
если мы рассматриваем правую часть  $x * (y + z)$. На самом деле, для более 
наглядной группировки  мы используем скобки даже когда {\tt prec} и приоритет 
текущего оператора равны. 
В случае ассоциативных операторов, вроде $+$ это не важно, но мы должны различать 
$x - (y - z)$ и $(x - y) - z$. (Более усложнённая схема могла бы учитывать 
ассоциативность каждого оператора.) Вторая, взаимно рекурсивная, функция
{\tt string\_of\_terms} используется для печати списка термов через запятую, как 
они идут в неинфиксной форме $f(t_1,\ldots,t_n)$.
...
На  самом деле, нам даже не требуется самостоятельно конвертировать терм в строку. CAML Light
позволяет нам настроить свою систему вывода. Это требует несколько специальных команд:
...
После того как новый принтер установлен, он будет использоваться всякий раз
при печати типа {\tt term}, даже если он представляет собой составной 
тип данных, как, например, кортеж:
...
Подобная печать всё же довольно груба, поскольку большие выражения не разбиваются аккуратно
на линии. Библиотека {\tt format}, из которой мы использовали несколько функций выше, 
предлагает лучшее решение. Вместо конвертации терма в строку и печати, мы можем 
выполнить отдельные вызовы печати для каждой его части, с помощью
специальной функции, помогающей системе печати разбивать строки. 
Многие из принципов, использующиеся в этой и подобных системах форматированного
вывода рассматриваются в \citeN{oppen-pretty}. Мы не будем рассматривать их в книге,
за подробностями обращайтесь к руководству по CAML.\footnote{Там также есть учебник на эту тему,
написанный Pierre Weis. Адрес:~{\tt http://caml.inria.fr/index-eng.html}}
...
Перейдём теперь к довольно простой задаче нахождения производной от функции. 
...
\begin{itemize}
  ...
  \item Если выражение - одна из элементарных функций дифференцируемого аргумента, 
  например \alert{$sin(x)$}, то результат дифференцирования~-- известная производная этой функции.
...
терминология редко используется в школах. Мы можем дословно запрограммировать его в ML:
...
Функция {\tt chain} предназначена просто для упрощения последовательного 
дифференцирования. Мы также систематически используем правила для сумм,
произведений и пр. а также производные стандартных функций. Конечно,  мы
могли бы, по желанию, добавить и другие функции, например, гиперболические 
или обратные тригонометрические. В паре случаев, а именно, для деления и 
функции~$tan$, мы избегаем усложнённого определения за счёт альтернативной формулировки.
...
Если мы опробуем {\tt differentiate} на нашем рабочем примере,  она будет 
...
Однако она не выполняет различные очевидные упрощения, такие как $0 * x = 0$
и $x + 0 = x$. Некоторые из этих избыточных выражений создаются нашей прямолинейной 
стратегией дифференцирования, которая, например, применяет правило
последовательного дифференцирования~$f(t)$ даже когда $t$~-- всего 
лишь переменная дифференцирования. Вместо избыточного усложнения функции 
дифференцирования давайте зададим отдельную функцию упрощения, которая будет 
избавляться от необязательных выражений.
...
Упрощение применяется, начиная с корня дерева выражения. Мы выполняем 
его в обходе снизу - вверх. 
% This will also bring negations upwards through products. 
% - что - то про знак "-"  
В некоторых случаях было бы более эффективно упрощение при обходе сверху~~- 
вниз, например, для $0 * t$ и сложного выражения $t$, но это потребует 
повторения в случае термов вроде $(0 + 0) * 2$. Выбор здесь напоминает выбор 
...
\noindent Теперь мы получаем лучшие результаты:
...
В общем, всегда можно добавить более усложнённые правила упрощения. 
Например, рассмотрим:
...
Мы хотим упростить {\tt cos(x) * inv(cos(x))} до {\tt 1},
пренебрегая, как это делает большинство коммерческих систем компьютерной
алгебры, возможностью обращения {\tt cos(x)} в нуль. Мы, конечно, можем 
добавить дополнительные правила упрощения. В то же время, можно было бы  
сгруппировать множители во втором слагаемом. И здесь, мы начинаем понимать,
имеет значение человеческая психология. Что предпочтительней?
Для машины самостоятельное решение будет затруднительно.
...
Выбрав второй вариант, затем следует заменить 
...
Такая запись и короче, и более привычна. Тем не менее, такое решение 
основывается на том, что мы знаем определения  довольно редко используемых 
тригонометрических функций, вроде {\tt sec}. Что бы машина не решила делать, 
вероятно, найдётся пользователь, которого это не устроит.
...
Формальный язык определяется своей {\em грамматикой}, которая задает множество 
...
допустимых имен и числовых констант, но мы могли бы также определить их
...
для каждой синтаксической категории вводится своя функция, причем структура 
...
нам потребуется вернуть в дополнение к результату разбора еще и список 
...
значением рекурсивного типа, введенного ранее. Отсюда мы имеем
...
Еще одна причина, по которой предложенный метод разбора особенно хорошо 
...
свертку относительно~{\verb!++!}, т.~е.\ применяет заданный анализатор 
...
%%% ISSUE: уточнить насчет приоритетов, что такое etc. в оригинале 
...
категории Other соответствуют обозначения операций и т.~п., причем в нашем
...
подходом к выводу выражений, следует учесть введенную ранее информацию о 
...
выражений, объединенных заданной операцией. Заметим, что того же эффекта
мы можем добиться при помощи нашего набора комбинаторов, но функция, 
приведенная ниже, проще и эффективнее.
...
\end{lstlisting}\end{boxed}
%
Обобщенный анализатор множества бинарных операций с приоритетами:
...
Прямое отображение этих правил вывода на язык~ML приведет к зацикливанию,
...
числа~$\pi$~\citeN{lindemann-pi}, он якобы отозвался о нем так: <<интересно,
...
возвращает масштабированное приближенное значение~$x$ с точностью~$2^{-n}$:
$$ 
 |f_x(n) - 2^n x| < 1.
$$
В свою очередь, данное выражение 
эквивалентно~$|{f_x(n) \over 2^n} - x| < {1 \over 2^n}$. Также возможно
непосредственное вычисление рационального приближенного значения, но для
...
арифметики. Предположим, что конфигурация системы задает следующие пути поиска
исполнимых файлов:
%
...
Перейдем к следующей задаче~--- сложению двух вещественных чисел. Предположим,
...
вычитания, но проще всего построить её на основе уже введенных функций:
...
$$
откуда делаем вывод, что~$x > y$. Прежде всего, приведем общую процедуру 
...
Проведенная оптимизация делает вычисление упомянутого выше произведения 
практически мгновенным. Рассмотрим еще несколько примеров:
...
В заключение отметим, что все расчеты, приведенные в данном разделе, можно,
...
нужно лишь приближенное значение результата. Однако, предложенный метод 
...
здесь рассматриваться не будет ввиду ограничений на объем курса, но может
...
Язык Пролог популярен в области исследований по Искусственному Интеллекту,
и используется в различных практических приложениях, таких как экспертные 
системы и интеллектуальные базы данных. В этой главе рассматривается как
различные механизмы Пролога, а именно: поиск вглубь с унификацией и возвратом,
могут быть реализованы в ML. Мы не претендуем на полную реализацию Пролога,
но результатов этого раздела достаточно, чтобы получить точное представление 
...
порядка. Мы уже определяли тип термов для математических выражений, их 
печати, и соответствующих синтаксических анализаторов. Сейчас мы будем 
...
{\verb+Fn(s,[])+}. Обратите внимание, функции различной арности
(разным числом аргументов)  рассматриваются как различные, даже если у них 
одинаковое имя. Следовательно нет опасности, что константы будут спутаны 
с настоящими функциями.
...
В целях следования соглашениям языка, которые включают чувствительность к
регистру, мы обновим функции лексического анализа. Не будем требовать
точного совпадения во всём, но учтём самое главное: 
символьно-цифровые идентификаторы, начинающиеся с буквы {\em в верхнем регистре}
или знака подчёркивания, рассматриваются как переменные, а другие 
символьно-цифровые идентификаторы и числа~-- как константы. Например, 
{\verb+X+} и {\verb+Answer+}~-- переменные,
тогда как {\verb+x+} и {\verb+john+}~-- константы. Символьные идентификаторы,
также как константы не различаются, в отличии от символов пунктуации: левой
и правой скобок, запятой и точки с запятой - их различать будем. 
Непунктуационные символы собираются вместе до строки наибольшей возможной 
длины.  
...
В Прологе <<.>> и <<nil>> соответствуют  <<{\tt ::}>> и <<{\tt []}>> в ML. 
Мы также допускаем прологовскую запись <<{\verb+[H|T]+}>> вместо 
<<{\verb+cons(H,T)+}>>,
используемую для сравнения по образцу. После основных функций:
...
\noindent у нас есть синтаксический анализатор для термов, а также правил
...
\noindent Собственно анализаторы:
...
Пролог использует набор правил для разрешения текущей {\em цели}. Для этого он
подбирает правила для цели. Правило, состоящее из одного терма, разрешает
цель немедленно, если оно совпадает с целью. В случае правила вида $term \mbox{
:- } term_1,\ldots,term_n\mbox{.}$ , если цель совпадает с $term$, то  
требуется разрешить, как подцель, по порядку каждый $term_i$. Цель является 
разрешимой, когда это выполнимо.
...
Однако, цели и правила не обязаны совпадать в точности до терма. В состав цели
и сопоставляемого правила могут входить переменные, и при сравнении цели с
правилом переменные могут присваивать (правильно было бы говорить связывать) 
соответствующие значения. Такое сравнение со связыванием называется 
{\em унификацией}. Оно означает, что мы можем пренебречь проверкой отдельных 
вариантов исходной цели, т.е. ограничиться $P(Y)$ вместо $P(f(X))$. Например:
...
\item Для унификации $f(g(X),Y)$ и $f(g(a),X)$ связываются
$X = a$ и $Y = a$. После этого термы будут: $f(g(a),a)$.

\item Для унификации $f(a,X,Y)$ и $f(X,a,Z)$ связываются 
$X = a$ и $Y = Z$, после чего оба терма примут вид $f(a,a,Z)$.

\item Недопустимо унифицировать $f(X)$ и $X$ ().
...
можно выбрать связывание $Y = f(b)$ и $Z = f(b)$. Однако, всегда 
...
Для того чтобы найти её, требуется рекурсивно (и синхронно) обходить 
вглубь оба терма, и при нахождении переменной в одном терме, связывать её 
с соответствующим подтермом другого терма. При этом также требуется следить, 
чтобы уже связанные переменные не унифицировались повторно с отличными 
термами и чтобы не возникало ситуаций, как в последнем примере, когда 
переменная, связываемая с термом входит в него же. Простая реализация этой 
идеи такова:
...
\noindent Аккумулятор переменных {\tt insts} должен пополнятся с 
<<проверкой наличия>>. {\tt http://en.wikipedia.org/wiki/Occurs\_check }
А именно, мы должны запретить связывания переменной $X$ с нетривиальным термом, 
включающем $X$, как в третьем примере выше. Большинство реализаций Пролога 
игнорируют подобную проверку или по (заявленным) причинам эффективности, 
или для поддержки циклических структур данных, более сложных чем простые 
...
\subsection{Перебор с возвратами}

Пролог выполняет поиск в глубину, но он может использовать возврат: допустим
правило успешно унифицирует цель, но последующие цели не могут быть
разрешены при текущих связываниях. Тогда выполняется возврат, связывание 
переменных освобождается, и с текущей целью сверяется другое правило.
...
Для генерации новых переменных используется функция {\tt rename} :
...
пытается разрешить цель, используя заданные правила: 
...
Результат функции~~- сообщение о том, что цель или не может быть разрешена, 
или разрешается с заданными связываниями. В последнем случае мы возврашаем 
только один ответ (один набор связанных переменных), но можно изменить код 
...
Давайте испытаем написанный интерпретатор на примерах из какой-нибудь книги 
по Прологу. Например:
...
использовать запросы в более гибкой форме, например спрашивать при каких
аргументах можно получать заданный результат:
...
Но за этой наружностью стоит простой поиск, а значит <<интеллектуальность>> 
хрупка. Например, следующий код зацикливается:
...
Пролог действует как простой доказыватель теорем, используя базу логических 
фактов (правил) для доказательства цели. Однако он весьма ограничен в 
доказательных возможностях, частично из-за неполноты стратегии поиска в 
глубину, частично потому что он может выполнять логические выводы только 
определённого вида. Вообще, можно реализовать более мощную пролого-подобную 
систему: \citeN{stickel-pttp}. Далее мы покажем, как в сущности на тех же 
идеях (унификация и возврат) строится более функциональный доказыватель теорем.
...
Унификация является эффективным способом ограничения переменных с навешенным 
квантором общности. Например, имея правила $\all{X} p(X) \Imp q(X)$ и 
$p(f(a))$, можно унифицировать оба выражения, включающие $p$ и тем самым 
связать $X$ с $f(a)$. Напротив, самые ранние системы доказательства теорем 
...
Обычно поиск вглубь может привести к бесконечному зацикливанию, так что
мы должны незначительно изменить стратегию поиска. Будем использовать
{\em поиск вглубь с итеративным погружением}. Это значит, что поиск вглубь 
имеет фиксировнный предел, по достижению которого выполняется 
возврат. Если доказательство не найдено при заданной глубине, её значение
увеличивается и делается ещё один поиск. Таким образом, сначала ищутся
доказательства глубины $1$, в случае провала~~- глубины $2$, $3$  и так далее.
...
Для обозначения формул будем использовать термы первого порядка. Добавим также
новые константы для логических операторов, некоторые из которых - инфиксные.
...
forall(X,p)      & для всех X, p  (квантор общности)                \\
...
Очевидно, программа упростится, если  основная часть доказывателя не будет 
работать с импликациями и эквивалентностями. Следовательно, сначала определим 
функцию, заменяющую их комбинацией других операторов.
...
элементарных термов и их отрицаний, связанных с помощью операторов 
<<И>>, <<ИЛИ>> и кванторов. 
...
Идея такова: отрицание доказываемой формулы конвертируется в негативно
нормальную формулу, с которой главный доказыватель попытается получить
противоречие. Этого противоречия будет достаточно для доказательства 
исходной формулы.
...
\subsubsection*{Главный доказыватель}

На каждом этапе доказыватель имеет текущую формулу, список формул, которые ещё 
предстоит рассмотреть и список литералов. Он пытается получить противоречие. 
...
делается попытка унифицировать его противоречивым литералом.

\item Если унификация не удалась, литерал добавляется в список литералов, 
а доказыватель переходит к обработке следующей формулы.
...
Нам необходима стратегия возврата, похожая на прологовскую: только
если текущие связывания позволяют разрешить все оставшиеся цели,
мы допускаем их. Можно было бы снова использовать списки, но давайте 
попробуем {\em продолжения}({\em continuations}).
Продолжение~~- это функция, передающаяся другой функции, которая
может вызываться из последней <<для выполнения остаточных вычислений>>.
В нашем случае она принимает список с переменными и пытается разрешить
оставшиеся цели при заданных связываниях. Следовательно вместо
попыток явно разрешить оставшиеся цели, мы просто вызываем 
функцию-продолжение.  
...
\noindent И, наконец, доказыватель:
...
Так реализуется стратегия итеративного погружения. Доказыватель пытается
найти доказательство с наименее общими связываниями переменных; если это 
удаётся, он возвращает  их число и сами значения переменных.
...
Вот несколько простых примеров, взятых из \citeN{pelletier-problems}.
...
\noindent Фактически доказыватель может работать детективом:
...
Символьное дифференцирование~~- классическое приложение для функциональных
языков. Другие символьные операции также используются~-- 
\citeN{davenport-ca} содержит обзор того, что могут делать системы компьютерной
алгебры и как они работают. В \citeN{paulson-rewriting} обсуждается
стратегия упрощения, использованная нами в более общих условиях. 
...
пример. Похоже он давно уже вошёл в в фольклор функционального программирования;
первоначально он был рассмотрен в \citeN{burge-recursive}. Наши примеры 
основываются на \citeN{paulson-ml} и \citeN{reade-efp}.
...
Первое определение <<вычислимого вещественного числа>> было дано в \citeN{turing}. 
Его подход основывался на десятичных расширениях, но позже потребовались 
изменения~--\cite{turing2}, по причинам, рассматриваемым далее в упражнениях. 
Подход к вещественной арифметике, осуществлённый нами, основывается на работе 
\citeN{boehm-cartwright}. Недавняя более эффективная реализация на CAML Light
была описана в \citeN{menisse-thesis}. Эта статья (на французском)
содержит подробное доказательство корректности всех алгоритмов для 
элементарных трансцендентных функций. Альтернативное решение использует
<<дробно-линейные преобразования>>, см. \citeN{potts-thesis}.
...
списков и продолжений, достаточно стандартен. За подробностями о реализации
Пролога обращайтесь, например, к \citeN{boizumault-prolog}, с подробностями о 
текущем использовании языка стоит ознакомиться по \citeN{clocksin-mellish}.
Детальное обсуждение продолжений, во многих их проявлениях, даётся 
в \citeN{reynolds-continuations}. Алгоритм унификации, расммотренный в 
курсе прост и достаточно функционален, но не блещет эффективностью.
За информацией о более быстрых императивных алгоритмах обращайтесь к
\citeN{martelli-montanari}. Наш доказыватель теорем основывается на 
\mbox{{\sf lean}$T^{\!\!\textstyle A}\!\!P$} \cite{beckert-posegga}.
О другом важном методе доказывания теорем, для удобства основывающемся
на прологовском стиле поиска, можно прочитать в \cite{stickel-pttp}.
...
когда $g(x) = 0$. В действительности большинство коммерческих систем 
компьютерной алгебры игнорирует это факт. Напишите новую версию
...
не позволит взять любой интеграл. \footnote{В действительности, есть 
алгоритмы, позволяющие интегрировать произвольное алгебраическое выражение 
(не содержащее $sin$, $ln$ и пр.) --- подробности в \citeN{davenport-integration}.}
...
\item Прочитайте документацию на библиотеку {\tt format}, и попробуйте 
реализовать форматированную печать многострочного вывода.
...
Что случится, если {\tt precedence} будет последовательно $\eta$-редуцироваться
...
\item На сколько хорошо анализатор с приоритетами, сгенерированный функцией
 ...
 запись допускала смешение лево- и правоассоциативных операторов?
...
Представьте вещественное число с помощью функции, генерирующей первые
% $n$ цифр в десятичной системе. Запрограммируйте функцию сложения.
...
\item (*) ???TODO спросить у мамы )
% \item (*) How can you retain a positional representation while making the basic
% operations computable?

\item Реализуйте мультиразрядную цклочисленную арифметику. Постройте алгоритм
произведения, выполняющийся за $O(n^{log_2 3})$ (??? времени памяти????), 
...
\item (*) Extend the real arithmetic routines для функций типа $exp$ и $sin$.
% \item (*) Extend the real arithmetic routines to some functions like $exp$ and
% $sin$.

\item Наша предподготовка и приведение к негативной нормальной форме могут
выполняться за время экспоненциальное от длины ввода, если ввод содержит
множество вхождений операторов эквивалентности. Модифицируйте функции так,
чтобы зависимость времени от числа операторов была линейной.
...
например в \citeN{enderton-logic}. TODO: вариант книги на русском} 
...
\item (*) Следуя, например, \citeN{martelli-montanari} реализуйте более
эффективный алгоритм унификации.
% \item (*) Implement a more efficient unification algorithm, e.g. following
% \citeN{martelli-montanari}.

\item (*) Реализуйте аналог Prolog Technology Theorem Prover.
