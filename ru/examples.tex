...
знания всех деталей, как, например, в случае анализа  приближения вещественных
чисел, приведённом далее. Однако, важно испытать эти программы и похожие примеры, а также, 
...
охватывает приложения занимающиеся, манипуляциями над математическими 
{\em выражениями}, в общем случае содержащими переменные, а не просто численными
расчётами. Есть несколько успешных <<систем компьютерной алгебры>> таких как
Maxima, Maple and Mathematica. Они они могут выполнять полезные
математические операции, такие как разложение многочленов на множители, 
дифференцирование и интегрирование выражений. (Они также пригодны и для
...
с определёнными производными основных функций, например ${d \over dx} sin(x) =
...
Мы выражаем это с помощью следующего рекурсивного типа данных:
...
{\em синтаксический анализатор (парсер)} и {\em системы вывода}, позволяющие
вводить и выводить данные в читаемой форме. Подробное обсуждение синтаксического 
разбора мы отложим, поскольку эта, сама по себе важная тема, будет обсуждаться 
далее, а сейчас мы напишем простенькую систему вывода для наших выражений. Такую 
чтобы, по крайней мере, мы могли видеть что происходит. Мы хотим, чтобы 
...
но для простых приложений, в котороых списки не слишком длинны, решение в виде линейного 
...
предпочитают динамическое связывание. Мы можем сделать множество инфиксных операторов 
...
Заметьте, по правилам вычисления в ML, разыменование указателя применяется только
после того, как {\tt get\_precedence} получает свои аргументы, и, следовательно, 
результаты в множестве операторов. Мы также можем определить функцию, определяющую,
...
Альтернатива заключается в использовании общей функции {\tt can}, которая проверяет успешна ли
другая функция:
...
скобки необходимы если приоритет этого оператора ниже {\tt prec}, иными словами, если мы
рассматриваем правую часть  $x * (y + z)$. Действительно, мы используем 
скобки даже когда они равны, для более наглядной группировки. 
В случае ассоциативных операторов, вроде $+$ это не важно, но мы должны различать 
$x - (y - z)$ и $(x - y) - z$. (Более усложнённая схема могла бы связывать с каждым
оператором ассоциативность.) Вторая, взаимно рекурсивная, функция
{\tt string\_of\_terms} используется для печати списка термов через запятую, как 
они идут в неинфексной форме $f(t_1,\ldots,t_n)$.
...
позволяет нам настроить свою систему вывода. Это потребует несколько специальных команд:
...
последовательного дифференцирования~$f(t)$ даже когда $t$~-~всего лишь переменная
дифференцирования. Вместо избыточного усложнения функции дифференцирования
давайте зададим отдельную функцию упрощения, которая будет избавляеться 
от необязательных выражений.
...
Мы просто применяем упрощение с самого вехнего уровня. Мы должны выполнять его
в обходе выражения снизу - вверх. 
% This will also bring negations upwards through products. 
% - что - то про знак "-"  
В некоторых случаях было бы более эффективно упрощение при обходе свеху - вниз, 
например, $0 * t$ для усложнённого выражения $t$, но это потребует повторения
для термов вроде $(0 + 0) * 2$. Выбор здесь напоминает выбор между
...
В обшем, всегда можно добавить более усложнённые правила упрощения. 
Например, рассмотрим:
...
вроде {\tt sec}. Чтобы машина не решила делать, вероятно, найдётся пользователь,
которого это не устроит.
