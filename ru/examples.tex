...
\#BUG: Ни о каких термах и речи нет. Сложности перевода???
...
Если мы опробуем {\tt differentiator} на наших рабочем примере,  она будет 
...
и $x + 0 = x$. Некоторые из этих избыточных выражений создаются скорее 
неизощрённой стратегией дифференцирования, которая, например, применяет правило
последовательного дифференцирования~$f(t)$ даже когда $t$~-~всего лишь переменная
дифференцирования. Вместо избыточного усложнения {\\tt differentiator} 
давайте зададим отдельную функцию упрощения, которая избавляется от необязательных выражений.
...
Мы просто применяем упрощение с самого вехнего уровня. We need to
...
операторам приоритетов и ассоциативности. Кроме того, мы можем добиться такого
...
Например, процедура анализа термов~{\verb!term!} при получении из входного
потока символа~{\verb!-!} будет рекурсивно вызывать саму себя для анализа
...
и~<<{\verb!96!}>>, так что входной поток состоит уже из лексем, о типе которых
...
В общем случае, анализатор может не обработать входной поток полностью, так что
...
считаются инфиксными операциями, так что во время определения операций мы
временно блокируем эту возможность при помощи ключевого слова~{\tt prefix}.
...
{\tt finished} предназначена для проверки того, что весь входной поток был
обработан.
