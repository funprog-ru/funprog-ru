...
символьно-цифровые идентификаторы и числа~-- как константы. Например, {\verb+X+} 
и {\verb+Answer+}~-- переменные,
...
длины, так что не рекомендуется использовать идентификаторы лишь из одного 
символа.  
...
\footnote{ правило $term \mbox{:- } term_1,\ldots,term_n\mbox{.}$ дословно означает:  
$term$ если $term_1$ и $term_2$, и \ldots  $term_n$  }
\end{eqnarray*}
...
Однако, цели и правила не обязаны совпадать в точности до терма. В состав цели и 
сопоставляемого правила могут входить переменные, и при сравнении цели с правилом
переменные могут присваивать (правильно было бы говорить связывать) соответствующие
значения. Такое сравнение со связыванием называется {\em унификацией}. 
Оно означает, что мы можем пренебречь проверкой отдельных вариантов исходной 
цели, т.е. ограничиться $P(Y)$ вместо $P(f(X))$. Например:
...
\item Для унификации $f(g(X),Y)$ и $f(g(a),X)$ мы можем связать
$X = a$ и $Y = a$. После этого термы будут: $f(g(a),a)$.

\item Для унификации $f(a,X,Y)$ и $f(X,a,Z)$ мы связываем 
...
В общем случае, унификация неоднозначна. Например, во втором примере мы
могли выбрать связывание $Y = f(b)$ и $Z = f(b)$. Однако, всегда 
...
\noindent Здесь аккумулятор переменных {\tt insts} позволяет избежать так 
называемой проблемы <<проверки наличия>>. 
http://en.wikipedia.org/wiki/Occurs_check 
Также мы должны запретить связывания переменной $X$ с нетривиальным термом, 
включающем $X$, как в третьем примере выше. Большинство реализаций Пролога 
игнорируют это или по (заявленным) причинам эффективности, или для поддержки
причудливых циклических структур данных, кроме простых термов первого порядка.  
...
разрешены при текущих связываниях. Тогда выполняется возврат и с текущей целью
сверяется другое правило.
