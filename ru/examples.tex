...
чисел, приведённом далее. Однако важно позапускать эти программы, примеры и 
...
имеет фиксировнную глубину~~- предел, по достижению которого выполняется 
...
В качестве <<глубины>> могут использоваться различные параметры, например,
высота или размер дерева поиска; мы будем использовать число унифицируемых
переменных.
...
Идея такова: отрицание доказываемой формулы будет конвертироваться в негативно
нормальную формулу, с которой главный доказыватель будет пытаться получить
противоречие. Этого будет достаточно для доказательства исходной формулы.
...
Следующая стратегия используются:
...
\begin{itemize}
\item Если текущая формула~-- {\em p {\verb+&+} q} то 
