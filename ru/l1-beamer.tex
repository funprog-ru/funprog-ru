...
записываемой как \mbox{{\red \tt v = E}} или \mbox{{\red \tt v := E}}.
...
возможно разделяя точкой с запятой: {\red $C_1 \mbox{ ; } C_2$}.
...
Программа стартует в вычислителе с начальным состоянием {\red $\sigma_0$},
...
После завершения программы, вычислитель оказывается в состоянии {\red
...
$\sigma_n$}; в принципе, каждая команда может изменить состояние.
...
\item Мы можем определять {\red рекурсивные функции}, создавая что-то
  наподобие циклов.
...
\item Более гибкое использование функций, например, мы можем
  определять {\red функции высшего порядка}.
