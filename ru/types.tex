...
возникает ощущение неполноты понимания формальной системы, в рамках которой
...
было продиктовано приведенными соображениями.
% 
...
модульности программ и скрытия информации путём разделения интерфейса некоторой 
структуры данных и деталей её внутреннего представления. 
%
...
полагая излишне утомительными ограничения на стиль программирования, 
накладываемые типизацией. Как следствие, различается и уровень её поддержки 
...
лишь {\em нестрогую типизацию,} которая допускает некоторые варианты 
...
типизация, напротив, может существенно снизить издержки.
%
...
сохраняет все выгоды строгой статической типизации, дополняя их
...
{\em строгой.} Терм~$t$ {\em обязан} иметь тип~$\sigma$, производные типы
...
Подобная нотация традиционно используется математиками при работе с 
...
считать типы множествами, которые содержат соотвествующие объекты, и 
трактовать~$t:\sigma$ как~$t \in \sigma$. Однако, несмотря на то, что мы
предлагаем читателям также воспользоваться этой удобной метафорой, 
...
согласуется с другими синтаксическими правилами, касающимися карринга.
%
...
констант) так называемые {\em переменные типа,} которые впоследствие лягут в
...
Принимая во внимание {\em free} inductive generation, можно доказать важное
свойство типов, а именно, что~$\sigma \to \tau \not= \sigma$. 
...
применения терма к самому себе, unless the two
...
подходящим <<нетипизированное лямбда-исчисление с separate
notion of type assignment>>}
...
существуют два взаимосвязанных аргумента в пользу данного подхода к типизации. 
...
оказываются достаточно сложными. Отношение typability не может быть задано
...
частичной функции, отображающей indexing множество переменных во множество 
...
интерпретировать как <<$t$ {\em может} быть назначен тип~$\sigma$>>.
...
определение отношения typability, так что терм может иметь тип лишь тогда,
когда он выводим при помощи упомянутых выше правил. В качестве примера 
...
преобразований не изменяется (такое свойство называется {\em type preservation}).
...
покажем, что добавление новых элементов в контекст не влияет на typability:
%
...
\proof 
Применим индукцию по структуре~$t$. Зафиксировав~$t$, докажем приведенное выше
...
откуда получаем требуемое. Если $t$~--- константа, обоснованность леммы
очевидна, поскольку множество констант и их типов не зависит от контекста.
В случае терма~$t$, имеющего вид~$s\; u$, для некоторого типа~$\tau$ 
...
представляет собой комбинацию вида~$s\; u$, то для некоторого~$\tau$ 
...
$x \not\in FV(t)$, то применив лемму, получаем~$\vdash t : \tau \to \tau'$,
...
полиморфизма все эти типы систематически связаны друг с другом, так что 
...
Предположим, что тип констант может быть им назначен в пустом контексте,
и что мы можем использовать двукратное применение правила типизации комбинации
для назначения типа~{\tt if} (принимая во внимание, что выражение 
вида~$\mbox{if}\; b\; \mbox{then}\; t_1\; \mbox{else}\; t_2$ является всего
лишь сокращенной записью для~$\mbox{COND}\; b\; t_1\; t_2$).
%
...
трактовки конструкции~{\tt let} как сокращенной записи в пользу реализации 
...
Typable выражения обычно имеют множество типов, хотя некоторые из них,
...
Более того, для каждого typeable выражения существует так называемый
{\em наиболее общий (основной) тип,} причём все возможные типы данного
...
так как не требует учета связывания переменных. Для удобства дальнейшего
...
соответствующих частных случаев. Опираясь на приведенное определение 
...
\begin{theorem}
Каждый typable терм имеет основной тип, т.~е.\ для произвольного~$t : \tau$
найдется тип~$\sigma$ такой, что~$t : \sigma$ и для любого типа~$\sigma'$ 
...
то есть, рефлексивно и транзитивно. Основной тип не уникален, но при этом все
...
одновременно основными типами выражения, то справедливо~$\sigma \sim \tau$,
...
Доказательство теоремы об основном типе не требует особых усилий, но ввиду
большого объема в рамках данного курса не рассматривается. Следует лишь 
...
\footnote{Приведенная нами формулировка теоремы была опубликована в работе
\citeN{milner-poly}, но как сама теорема, так и алгоритм были ранее открыты
...
выражений автоматически выводится их основной тип либо устанавливается 
...
на основании теоремы о {\em строгой нормализации,} доказательство которой
слишком длинно, чтобы быть здесь приведенным.
...
\begin{theorem}
Любой typable терм имеет нормальную форму, а любая возможная последовательность 
редукций, которая начинается с typable терма, завершается.%
\footnote{Под <<нестрогой нормализацией>> подразумевается первая часть данного
...
остается справедливой и в случае типизированного лямбда-исчисления.) Однако,
...
определенных вычислимых функций, мы всегда можем определить еще одну, не
...
функции, более того --- даже не все множество всюду определенных функций.
...
оказывается весьма узким. В работе~\citeN{schwichtenberg-definable} показано,
что класс представимых функций на основе нумералов Чёрча ограничен всевозможными
полиномами либо result from polynomials using definition by cases.
...
Поскольку все определимые функции являются всюду определенными, мы, очевидно,
не в состоянии давать произвольные рекурсивные определения. (???)
В самом деле, оказывается, что обыкновенные комбинаторы фиксированной точки
...
Типизированное лямбда-исчисление рассматривается, в числе прочего, в 
работах~\citeN{barendregt} и~\citeN{hindley-seldin}. Основополагающая
работа~\citeN{milner-poly} продолжает оставаться хорошим источником
информации о полиморфной типизации, включая алгоритм поиска основного типа.
Хорошее элементарное введение в типизированное лямбда-исчисление, включающее
доказательство теоремы о строгой нормализации, а также обсуждение некоторых
интересных взаимосвязей с логикой, даётся в~\citeN{girard-prat}. 
...
большинства требуемых на практике функций даже при сохранении свойства строгой 
нормализации.
%
...
\item 
Докажите формально теорему о type preservation для $\alpha$ 
и~$\beta$-преобразования.
%\item Prove formally the type preservation theorems for $\alpha$ and $\beta$
%conversion.

\item 
Покажите, что свойство type preservation необратимо, т.~е.\ что возможна
...
\item (*) 
Докажите, что каждый терм типизированного лямбда-исчисления, чей {\em основной} 
...
\footnote{Подробнее по данному вопросу см.~\citeN{mairson-param}.}
% 
...
\item (*) 
Будем говорить, что некоторое reduction relation $\goesto$  
{\em weakly Church-Rosser}, если всякий раз, когда~$t \goesto t_1$ 
и~$t \goesto t_2$, найдется~$u$ такое, что~$t_1 \goesto^* u$ 
...
утверждается, что если отношение is weakly Church-Rosser и удовлетворяет 
принципам строгой нормализации, то~$\goesto^*$ is Church-Rosser. 
(Указание: воспользуйтесь трансфинитной индукцией.)%
\footnote{Если в ходе доказательства возникнут затруднения, 
см. работу~\citeN{huet-reduct}.}
%
