...
\newpagestyle{ColourDemo}%
  {\cyanonblack{Введение в Функциональное Программирование:
                Лекция 5}\hfil\cyanonblack{\thepage}}
  {\cyanonblack{Джон Харрисон}\hfil
   \cyanonblack{Университет Кембриджа, 24 января 1997}}
%\newpagestyle{ColourDemo}%
...
\item Основное взаимодействие; выражения и объявления
...
\heading{От $\lambda$-исчисления к ML}
...
\item Примитивную конструкцию {\tt let}, чтобы полиморфическая система
...
Так будет продолжаться пока мы не построим ML.  Особенно
...
Например, преобразование {\red $2 + 2 \goesto 4$} происходит благодаря машинной
...
\heading{Проблема с нормальным порядком}
...
эффективнее сначала вычислить аргумент {\red $10 + 5$}.
...
\heading{Ленивое и энергичное вычисление}
...
проблемы, которые разбивают мир функционального программирования:
...
\item {\bf Ленивое вычисление} выполняет нормальный порядок редукции,
  но также использует хитрую технику разделения выражений, сохраняя их в
  виде графа.  При таком способе выражения вычисляются только один раз
  и только когда это понадобится.
...
\heading{Стратегия вычисления в ML}
...
\item При вычислении комбинации {\red $s\; t$} {\em сначала}
  вычисляются оба {\red $s$} и {\red $t$}.  Потом, при условии, что
  вычисление {\red $s$} дало $\lambda$-абстракцию, производится
  самая внешняя $\beta$-конверсия, и процесс продолжается.
...
\heading{Условное выражение}
%\heading{The conditional}

\vspace*{0.5cm}

Из-за энергичного вычисления нам необходимо создать примитив~--
условное выражение с собственной редукционной стратегией.  Если мы
...
Если мы хотим вычислить {\red $\mbox{fact}(0)$}, то нам надо будет
вычислить {\red $\mbox{fact}(\mbox{PRE}\; 0)$}, {\red
...
\item Мы делаем условное выражение примитивой языка и изменяем
  стандартную стратегию вычислений для него таким образом, что {\em сначала}
  вычисляется булево выражение, а потом {\em только один} из нужных
  рукавов.
...
Теперь просто наберите {\black \tt camllight}.  Система запустится и
выведет приглашение.
...
Если вы хотите поставить CAML Light на свой компьютер, зайдите на
страничку:
...
Самый простой способ взаимодействия с ML это использование его в роли
...
ML не только печатает результат, но также выводит {\em тип} выражения,
в данном случае {\black \tt int}.  Если ML не может назначить тип
выражению, то оно не пригодно.
...
ML знает тип встроенного оператора {\black \tt +} и именно поэтому он
...
\heading{Используем функции (1)}
%\heading{Using functions (1)}

\vspace*{0.5cm}

Так как ML это функциональный язык, то выражения могут иметь
функциональный тип.  Синтаксис ML для обозначения $\lambda$-абстракции
{\red $\lamb{x} t[x]$} выглядит как {\black \tt fun x -> t[x]}.
Например мы можем задать функцию инкремента:
...
Опять же, тип выражения, в этот раз {\black \tt int -> int}, был выведен
и показан.  Однако сама функция не была напечатана; система просто
написала {\black \tt <fun>}.
...
\heading{Используем функции (2)}
%\heading{Using functions (2)}

\vspace*{0.5cm}

ML поддерживает синтаксис подобный принятому в $\lambda$-исчислении
...
Когда связываются функции, можно использовать более элегантную обёртку:
...
По сути, набор связываний может быть дополнен интерактивно.  Просто
опустите {\black \tt in} и завершите выражение:
...
Таким образом можно создать набор объявлений функций, которые могут
быть использованы позже в сеансе работы.
...
\heading{Полиморфические функции}
%\heading{Polymorphic functions}

\vspace*{0.5cm}

Мы можем задавать полиморфические функции, такие как оператор
тождества:
...
ML печатает тип переменных как {\black \tt 'a},{\black \tt 'b} и так
далее.  Предполагается, что они являются ASCII представлением для
{\black $\alpha$}, {\black $\beta$} и так далее.  Теперь мы можем
использовать тождество с разными типами:
...
В последнем выражении все вхождения {\black \tt I} имеют различные
типы и интуитивно соответствуют разным функциям.
...
Заметим, что система следит за типами без нас.  Вспомним что {\red $I
...
\end{verbatim}
\end{black}
У него правильный тип, и похоже он работает:
...
В основном {\em запрещено} сравнивать функции на равенство.  Почему?
Ведь предполагается, что функции являются объектами высшего порядка в ML?
...
Да, но к сожалению, (экстенсиональное) сравнение функций не
вычислимо.  Это следствие из {\em теоремы Риса}, или просто из {\em
...
Некоторые версии ML различают функции и другие `сравнимые
типы' и фильтруют такие случаи во время проверки типов.
...
\heading{Отклонения в проверке типов}
...
вариант, предложенный Майерсоном:
...
печать займёт свыше 4000 строк на 80-ти столбцевом терминале.
