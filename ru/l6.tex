...
системы, или редактор, с поддержкой буферов, такой как Emacs.
...
Используя команду {\black \tt include} для этого файла, мы получаем
следующее:
...
У многих встроенных констант, таких как, {\black \tt +}, есть
статус инфиксной операции.  Есть также понятие старшинства операции,
поэтому выражения связываются более или менее как ожидается пользователем.
...
{\black \tt \&}   & Логическое `и'         \\
%{\black \tt \&}   & Boolean `and'         \\
{\black \tt or}   & Логическое `или'          \\
...
рассматривать их как синонимы для:
...
так что {\black \tt not not p} разбирается как {\black \tt not (not
...
положительное число {\red $n$} и функцию {\red $f$}, а возвращает
{\red $f^n$}, то есть {\red $f \circ \cdots \circ f$} ({\red $n$}
раз):
...
На самом деле {\black \tt funpow} переводит машинное число {\black \tt
  n} в соответствующий нумерал Чёрча.  Мы легко можем определить
...
\item Они инъективны, т.е. равенство {\black \tt inl x = inl
...
Вообще, мы можем определать частичную функцию, которая определена не
...
функция которая добавляет элемент в начало списка.  Списки {\black \tt
...
\heading{Сопоставление с образцом для списков}
...
выглядит естественно, поскольку сами типы были заданы рекурсивно.  Вот
...
определённые `деструкторные' функции {\black \tt hd} и {\black \tt
  tl}.  Такой стиль определения функций принят во многих языках,
особенно в LISP, однако прямое использование сопоставления с образцом
зачастую элегантнее.
