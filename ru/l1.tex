...
\item $\lambda$-нотация и её польза
...
помощью последовательности {\em команд} для исполнения.
...
конечное время для выработки результата.
...
$\sigma_n$}, с учётом вывода программы.
...
Но каковы конкретные причины, чтобы предпочесть функциональные
языки?
...
Без переменных и присваивания мы имеем следующие преимущества:
...
\item Б\'{о}льшая свобода исполнения, т.е. распараллеливаемость.
...
необходимо чётко определить состояние.  Например, мы можем задать
команды как:
...
Функциональное программирование не без недостатков.  Некоторые вещи
сложнее всунуть в чисто функциональную модель, типа:
...
Функциональные языки также меньше соответствуют современному железу,
поэтому они могут быть менее эффективны и в них может быть сложнее
обосновать время исполнения и затраченные ресурсы.
...
ML не чисто функциональный язык, поэтому вы можете присваивать
переменные, если потребуется.  Большинство наших программ написано на
чисто функциональной его части.
...
$\lambda$-нотация - это способ задавать функции, изобретённый Алонзо
...
$E[x]$}'.  Где {\red $E[x]$} это выражение, которое содержит или не
содержит {\red $x$}.
...
Литера {\red $\lambda$} - историческая случайность. Изначально Чёрч
использовал {\red $\hat{x}.\; E[x]$}, а серия ошибок набора превратило
это в нынешнюю форму. Иногда также пишут {\red $x \mapsto E[x]$} и
{\red $[x]\; E[x]$}.
...
В неформальной математике, когда мы говорим о функции, то обычно именуем
её: `Пусть {\red $f(x) = E[x]$} \ldots тогда {\red $\cdots f
\cdots$}'.
...
только, если она будет именована.
...
важно в функциональном программировании.
...
$\lambda$-нотация также полезна для улучшения ясности математики.
...
$\lambda$-нотацию мы можем задать это явно.
...
\item Переменные, то есть {\red $x$}, {\red $y$}.

%\item Variables, e.g. {\red $x$}, {\red $y$}.

\item Константы, то есть {\red $3$}, {\red $true$}, {\red $+$}.

%\item Constants, e.g. {\red $3$}, {\red $true$}, {\red $+$}.

\item Применение функции к аргументам, то есть {\red $f\; x$}.

%\item Applications of functions to arguments, e.g. {\red $f\; x$}.

\item $\lambda$-абстракция выражения над переменными, то есть {\red
      $\lamb{x} x + y$}
...
аргументу, то есть {\red $f\; x\; y$} означает {\red $(f(x))(y)$}.
Это поддерживает каррирование, то есть
...
математики, введя константы для логических операций, то есть чтобы
