...
изменение значений наборов переменных, называемых {\em
...
выполняться последовательно, путем их последовательно записи в
программе, зачастую разделяя их с помощью точки с запитой.  С помощью
...
поддерживающие данный стиль, также известны как императивные или
процедурные языки.
...
`???(applicative programming)', поскольку основной механизм~-- это
{\em применение??? application} функции к аргументам.}  В общих чертах
мы можем увидеть как это возможно, используя следующие рассуждения.
Предположив, что императивная программа (вся целиком) детерминична
...
f(\sigma)$.\footnote{Сравните замечание Naur??? \cite{raphael-spl},
  что мы можем записать любую программу в виде одного выражения
  $Output = Program(Input)$.}  В функциональном программировании эта
точка зрения выявлена более явно: программа~-- это выражение, которое
соответствует математической функции $f$.  Функциональные языки
программирования поддерживают создание таких выражений, путем
использования можных функциональных возможностей???

Функциональное программирование может контрастировать с императивным
как в положительных, так и отрицательных вещах.  К негативным вещам
можно отнести то, что функциональные программа не используют
переменные~-- т.е., {\em не имеют} состояния.  Соответственно, они не
могут использовать присваивание, поскольку не во что присваивать.
...
могут использовать функции более сложным способом.  Функции могут
...
традиционных языков программирования обеспечивают весьма бедные
возможности в этих областях. Язык C имеет некоторые ограниченные
возможности манипуляции функциями с помощью указателей, но не
...
\noindent в то время как, эта фкнция может быть записана на языке ML
...
функциональные языке относятся к отдельному классу.
...
написанных тут.  Но мы надеемся, что изучая материал, приведенный
