...
входной последовательности символа {\black \verb!-!} будет рекурсивно вызывать себя для
...
\heading{Анализатор термов (вариант~1)}
...
Возможности языка~ML позволяют на основе динамического списка инфиксных
операций легко сконструировать <<исправленную>> с их учётом версию грамматики.
...
Использование этой функции в основном анализаторе делает его более простым
и более общим.
...
\heading{Анализатор термов (вариант~2)}
...
Эти определения дают возможность динамического конструирования анализатора
с учётом приоритетов на основе списка инфиксных операций в том виде, в котором
он существует в момент обращения к нему. Базовая грамматика упрощается.
...
\heading{Возврат и повторное сканирование}
...
\heading{Устранение избыточности}
...
\heading{Анализатор термов (вариант~3)}
