...
В этой главе закрепим предыдущие примеры более точно определив синтаксис и
основные возможности ML, а затем перейдём к рассмотрению некоторых дополнительных 
возможностей, таких как рекурсивные типы. Но начнём мы с обсуждения взаимодействия 
с системой. 
...
До сих пор мы печатали команды в интерпретатор, получая от него результат. 
Однако, это не лучший способ писать нетривиальные программы. Обычно вам
следует записывать все выражения и объявления в файл.
После этого повторное использование кода обеспечивается операцией `вырезать и вставить` 
Эта операция может быть выполнена  с помощью X-Windows и схожих систем, 
или в редакторе типа Emacs. Такой подход, однако, становится кропотливым и неэффективным 
для больших программ. Вместо этого вы можете использовать функцию {\tt include}
для непосредственного чтения из файла.
Например, если файл {\tt myprog.ml} содержит: 
...
\noindent то введя в интерперетатор {\tt include "myprog.ml";;} вы получите:
...
В больших программах часто полезно включать комментарии. В ML они записываются
между символами {\tt (*} и {\tt *)}, например.
...
ML представляет несколько встроенных примитивных типов. Из них, с помощью
конструкторов типов могут быть построены составные типы. Пока мы будем использовать
только конструктор функций  {\verb+->+} и конструктор декартового произведения {\verb+*+},
но в своё время мы рассмотрим и другие конструкторы, а также как определять
новые типы и конструкторы типов. Примитивные типы, которые интересуют нас, это:
...
\item Тип {\tt unit}, можно называть его процедурным. 
Это одноэлементый тип, единственный элемент которого записывается как {\tt ()}. 
Очевидно, использование типа {\tt unit} не передаёт какой - либо информации,
так  что он часто используется как возвращаемый тип императивно записанных
`функций', результат которых не значение вычислений, а какой -либо побочный эффект. 
Например, таких как {\tt include}.  Это также подходящий тип для аргумента  
FIX (?It is also a convenient argument where the only use of a function type is to delay
evaluation.?)
...
\item Тип {\tt bool}. Двухэлементный тип булевых значений (значений истинности).
Элементы этого типа: {\tt true} и {\tt false}.

%%\item The type {\tt bool}. This is a 2-element type of booleans (truth-values)
%%whose elements are written {\tt true} and {\tt false}.

\item Тип {\tt int}. Содержит конечное подмножество отрицательных и неотрицательных
чисел. Обычно значения варьируются от $-2^{30}$ ($-1073741824$) до $2^{30}-1$ ($1073741823$).
\footnote{This is even more limited than expected for machine arithmetic because a bit is stolen for the
garbage collector. We will see later how to use an alternative type of integers
with unlimited precision.} Элементы записываются обычным способом,
например: {\tt 0}, {\tt 32}, {\tt -25}.
