...
Такой подход, однако, становится утомительным и неэффективным для
...
\item Тип {\tt unit}, можно называть его процедурным.  Это
  ...
  Например, таких как {\tt include}.  Он так же может использоваться
...
Значения, такие как {\tt ()}, {\tt false}, {\tt 7} и
 \lstinline!"caml"!,  в смысле лямбда~-~исчисления рассматриваются 
...
В ML нет возможностей задавать приоритет для инфиксных операций,
 ...
 как правоассоциативные. Также запомните, что неявная
 ...
 функцию с особым статусом как обычную константу, вам потребуется {\tt prefix}:
...
конструкции, которые нами не будут использоваться. За подробностями обращайтесь 
к руководству по CAML.}
...
Представляется полезным разбор этого объявления в соответствии с приведённой
выше грамматикой. 
...
Не сложно задать рекурсивную функцию, принимающую в качестве аргументов целочисленное~$n$
...
В действительности, небольшое размышление показывает, что функция~{\tt
  funpow} принимает машинное целое~$n$ и возвращает нумерал (число)
Чёрча, соответствующее~$n$. Поскольку функции не выводятся на печать,
мы не можем в действительности посмотреть на лямбда-выражение,
представляющее число Чёрча:
...
Также просто задаётся функция обратная к {\tt funpow}, которая принимает число
...
\noindent Мы можем проверить некоторые из арифметических операций на числах Чёрча:
...
Примеры выше, очевидно, не являются самым эффективным способом выполнения 
арифметических операций. Далее, ML не имеет функции для возведения в степень, 
но её легко задать по рекурсии:
...
Это решение выполняет $n$~произведений для вычисления~$x^n$. Более эффективный
способ использует тот факт, что $x^{2 n} = (x^n)^2$ и $x^{2 n + 1} = x
(x^n)^2$:
...
Другая классическая операция на натуральных числах - нахождение их
...
Мы использовали  оператор рекурсии {\tt Rec} при объяснении рекурсивных определений. 
Действительно, давайте определим его:
...
создаётся из существующих типов и себя.  Мы покажем это на нескольких
примерах.  Первым примером будет определение типа {\em sum}, которые
...
$(\alpha,\beta)sum$ создаётся из точных отдельных копий $\alpha$ и
...
\noindent Эта функция имеет свойство (достаточно натуральное), что когда она применяется к
...
Сравнение по образцу может комбинироваться с рекурсией. Например, вот функция,
...
в LISP, но непосредственное сравнение по образцу часто бывает более элегантно.
...
Часто бывает полезно представлять части рекурсивных типов в виде дерева с
...
Точно так мы можем по рекурсии определить подстановку. Сначала, во избежание 
...
заданном выражении. Переименование производится путём добавления символа.
