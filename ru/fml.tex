...
В этой главе закрепим предыдущие примеры, более точно определив
...
начнём мы с обсуждения взаимодействия с системой.
...
содержимое файла в консоль. Последняя линия вывода~-- это результат 
...
\item Тип {\tt int}. Содержит конечное подмножество отрицательных и
  ...
  ($-1073741824$) до $2^{30}-1$ ($1073741823$).  \footnote{This is
    even more limited than expected for machine arithmetic because a
    bit is stolen for the garbage collector. We will see later how to
    use an alternative type of integers with unlimited precision.}
  Элементы записываются обычным способом, например: {\tt 0}, {\tt 32},
  {\tt -25}.
