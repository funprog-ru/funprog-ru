...
мы начнем работать.  $\lambda$-абстрация выглядит, ретроспективно, как подходящий примитив
...
увидим в следующей главе, Landin указывал на то, как много конструкций в языках
...
Martin-L\"of, и часто на нее ссылаются как на `теорию Martin-L\"of для выражений и арности
...
Как мы уже упоминали, одной из привлекательной сторон $\lambda$-нотации является то, что
она разрешает анализ почти всего математического синтаксиса.  Первоначально, Черч надеялся
...
математики.  Взяв любое множество~$S$, мы можем сформировать для него так называемый {\em
  параметрический предикат (characteristic predicate)} $\chi_S$, такой, что:
...
рассмотрения~$S$ как множества, и записи $x \in S$, мы можем рассматривать его как
предикат, и записывать как~$S(x)$.

Это позволяет проведение обычного анализа в $\lambda$-нотации: мы можем допустить что
произвольные $\lambda$-выражения являются функциями, и косвенным образом, и множествами.
К сожалению, это приводит к несовместимости.  Простейшим способом убедиться в этом,
является рассмотрение парадокса Рассела про множество всех множеств, которые не содержат
сами себя:
...
находим, что $R\; R = \Not(R\; R)$, рассчитывая на интуитивное понимание операции отрицания
$\Not$.

Для того, чтобы избежать таких парадоксов, \citeN{church-types} развивает идею Рассела
путем добавления в $\lambda$-нотацию понятия {\em типа}; мы должны рассмотреть это в
следующей главе.  Однако сам парадокс предлагает некоторые интересные возможности в
стандартной, нетипизированной системе, в чем мы убедимся позже.
