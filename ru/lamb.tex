...
(например, через указатель), то несмотря на её простоту, нам все равно
...
что из контекста понятно, о чём идет речь~--- о самой функции~$f$, или о результате её
применения к конкретному~$x$.  Дополнительной пользой будет то, что $\lambda$-нотация даёт
нам возможности анализа почти всего языка математики.  Если мы начнем с переменных и
...
посвященных функциям многих переменных, зачастую подчеркивают это различие
...
`абстрактного синтаксиса'; все что нам нужно~-- соответствующий набор констант, с которыми
...
Мартином-Лёфом, и часто на нее ссылаются как на `теорию Мартина-Лёфа для выражений и арности
...
её пригодность для анализа почти всего математического синтаксиса.  Первоначально, Черч надеялся
...
определенных с помощью $\lambda$-нотации, мы задаем $R = \lamb{x} \Not (x\; x)$, а затем
...
как правило, мы будем использовать в качестве имен буквы, расположенные
