...
математика-логика\citeN{curry-comb}. (Actually the device had previously been used by both
\citeN{frege-arith} and \citeN{schonfinkel}, but it's easy to understand why the
corresponding appellations haven't caught the public imagination.) The idea is to use
expressions like $\lamb{x\;y} x + y$. This may be regarded as a function $\real \to (\real
\to \real)$, so it is said to be a `higher order function' or `functional' since when
applied to one argument, it yields another function, which then accepts the second
argument. In a sense, it takes its arguments one at a time rather than both together. So
we have for example:
...
\subsection[Равенство лямбда-выражений]{Равенство $\lambda$}
...
эквивалентности~$=$ представляет собой симметричное транзитивное замыкание
...
то~$t_1\equiv_{\alpha} t_2$, т.~е.\ $t_1$ и~$t_2$ эквивалентны с точностью
...
редукции, в которой на каждом шаге выбирается внешний редекс, расположенный 
в выражении левее других, считается, в известном смысле, наилучшей, поскольку
