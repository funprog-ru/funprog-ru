...
\texttt{successor} (которая добавляет $1$ к своему аргументу) не самым простым способом
(например, используя указатель на нее), то несмотря на простоту, все равно
нам придётся назвать её, предварительно задав определение, такое как:
...
В математике или программировании это выглядит нормальным, и в общем случае, работает
...
то требование их именования будет нелогичным.  При обсуждении арифметического
выражения, построенного из более простых выражений, мы просто записываем подвыражения, не
давая им имен.  Представим, что было бы, если бы мы всегда работали с арифметическими выражениями
таким способом:
...
t[x]$.)  Вероятно, что она возникла во время сложного процесса эволюции.  Первоначально в
...
аргументом, то мы сможем представлять очень сложные математические выражения.
...
математика-логика \citeN{curry-comb}. (В действительности, такой приём уже использовался и
Фреге~\citeN{frege-arith}, и Шейнфинкелем~\citeN{schonfinkel}, но легко понять, почему соответствующие
термины не получили общественного признания.)  Идея заключается в использовании
...
В логике кванторы~$\all{x} P[x]$ (`для всех $x$, $P[x]$') и~$\ex{x} P[x]$
(`существует такой $x$, для которого $P[x]$~истинно') являются дополнительными примерами, а в теории
...
произошедших от Algol 60.  Переменные имеют определенную область видимости, а формальные
...
Между прочим, заметьте, что {\em область видимости} переменной не следует отождествлять с её
{\em временем жизни}.  В функции {\tt rand} на языке~C, которую мы привели в введении,
...
процесс преобразования одного выражения в другое, называется альфа-преобразованием.  Следует
...
\noindent $x$ используется и как связанная переменная для того, чтобы показать, что
...
путём добавления в $\lambda$-нотацию понятия {\em типа}; мы должны рассмотреть это в
следующей главе.  Однако, сам парадокс предлагает некоторые интересные возможности в
...
Некоторые очевидные факты принимались нами без обоснования, например, 
...
которые могут производиться, не задумывась.
%
