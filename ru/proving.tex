...
точности то, что требуется. Подавляющее большинство крупных программ содержит
...
программное обеспечение кардиостимуляторов, автопилотов, систем управления
двигателями, антиблокировочных тормозных систем, аппаратуры радиационной 
...
сферы жизни человечества, тем серьезнее возможная угроза жизни людей, 
...
много, чтобы полностью проверить все из них, так что многие ошибки могут быть 
пропущены в ходе тестирования. Как неоднократно подчеркивают многие авторы,
...
Более надежный подход~--- математическое {\em доказательство} справедливости
приведенного утверждения, что легко достигается путем индукции по 
...
ошибками в их реализации, а ложным пониманием ожиданий пользователей. Попытка
...
т.~е.\ между реальными требованиями к программе и её реальной реализацией.
Чтобы добиться этой цели, нам придётся формализовать и то, и другое. Лишь
одна связь на диаграмме, между математическими моделями, обладает 
математической же точностью, прочие связи остаются неформальными. Всё, что 
мы можем с ними сделать~--- использовать реалистичную модель системы и 
высокуровневую достаточно читабельную спецификацию.
...
раз и навсегда, причем, возможно, для целого класса программ одновременно
...
к объему доступной памяти. Таким образом легкость доказательства может 
...
В предыдущем разделе был приведен пример, который демонстрирует неточность
наивного отождествления of function spaces of ML and function spaces of 
mathematics (??). Однако, если мы не будем углубляться в подробности теории
рекурсивных типов и не рассматриваем пространство {\em произвольных} функций,
а ограничимся лишь некоторыми из них, то в этом случае возможно отождествление
понятий языка~ML с математическими.
...
неопределенными. Анализ всюду определённых функций менее трудоёмок, поэтому
иногда вводится дополнительный аргумент (???), гарантирующий завершимость.
...
(включая завершимость), определенных рекурсивно, можно доказать по индукции 
...
\section{Вычисление наибольшего общего делителя}
...
чисел~$u$ и~$v$, для которых `$v$ кратно~$u$', т.~е.\ найдется такое целое
...
рекурсии. На деле, {\tt gcd x y} выражается пошагово 
...
доказанной, поскольку для любого~$y$ найдется {\em некоторое}~$n$ такое,
...
общим делителем. Предположим, что найдется ещё один общий делитель~$d$,
...
предположение к~$\mbox{gcd}\; y\; (x\; \mbox{mod}\; y)$. Введем сокращенное
обозначение~$r = x\; \mbox{mod}\; y$. Основным свойством функции~{\tt mod} 
является то, что при~$y \not= 0$ для некоторого целого числа~$q$ мы 
имеем~$x = q y + r$ и~$|r| < |y|$. Поскольку~$|r| < |y|$, из индуктивного
...
Данная функция вводится при помощи примитивной рекурсии over the type of lists.
Она определяется для случая пустого списка и, затем, для списка вида~$h::t$,
причем в последнем случае используется значение этой же функции для аргумента~$t$.
...
В работе~\citeN{neumann-risks} приведен обзор рисков, связанных как с 
распространением компьютеров в обществе в целом, так и с последствиями
программных ошибок в частности. Очень интересная дискуссия с примерами 
на эту же тему даётся в~\citeN{peterson-defect}. Применимость верификации
являлась в своё время предметом споров; некоторые аргументы против этого
подхода изложены в~\citeN{demillo-social}. Также заслуживает внимания 
обоснованное обсуждение~\citeN{barwise-correctness}. На данный момент 
...
программирования для начинающих, такие как~\citeN{paulson-ml} 
и~\citeN{reade-efp}, включают базовые примеры наподобие рассмотренных в данной
книге. Представляет интерес работа~\citeN{boyer-acl}, авторы которой провели
...
в синтезе СБИС~\citeN{aagaard-pbs}.
