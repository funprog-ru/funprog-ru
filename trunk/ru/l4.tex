...
\frame{
  ...
  \item Сильная нормализация; рекурсия.
...
\frame{
  \frametitle{Различные способы типизации}
...
\frame{
  ...
  Однако предпочительнее использовать {\em неявную} типизацию.  По
...
\frame{
  \frametitle{Типизация по Карри}
  % Curry typing

  Здесь термы такие же, как и были раньше; кроме того, есть отдельное
  понятие типизации.  Всё типизируется {\em неявно}, и у терма может
  быть много типов.  Мы используем {\em контекст} \alert{$\Gamma$},
  который задаёт назначение типов для переменных и констант:

  \alert{$$ \Gamma \vdash t : \sigma $$}

  это означает, что <<при условии назначения типов из набора \alert{$\Gamma$},
...
\frame{
  \frametitle{Формальные правила типизации}
  % Formal typing rules
...
\frame{
  ...
  вхождений одной и той же переменной.  В типизации по Чёрчу это бы не
...
\frame{
  ...
  Это означает, что статическая типизация и динамическое исполнение не
  мешают друг другу.
...
\frame{
  ...
  Типизация по Карри уже даёт нам форму {\em полиморфизма}.  В том
...
\frame{
  \frametitle{Сильная нормализация}
  ...
  свойство называется {\em сильной нормализацией}.
