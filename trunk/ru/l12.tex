...
\section*{Лекция 12. Примеры на ML IV: Пролог и Доказательство Теорем}

\frame{
  \frametitle{Темы}
  \begin{itemize}
   \item Выражения в Прологе: анализ и печать
%  \item Prolog terms: parsing and printing
    \maybepause

  \item Унификация и поиск с возвратом в прологе
%  \item Unification and Prolog backtracking
    \maybepause

  \item Примеры на Прологе
...
%  \frametitle{Overview}
  ...
  Пролог это <<декларативный>> язык программирования популярный
...
%  Intelligence>>.
  ...
  Это логический язык программирования, и говорится, что он
  <<реляционный>>, а не функциональный.
%  It is a logic programming language, and is said to be <<relational>> rather than
%  functional.

  Его стратегия поиска, обратный (логический) вывод с унификацией и
  поиском с возвратом, являются полезными для широкого круга
  приложений, таких как: базы данных, экспертные системы и средства
...
%  expert systems and theorem provers.
  ...
  Мы покажем, как на ML сделать урезанный вариант интерпретатора
  Пролог.
%  We will show how to implement a cut-down Prolog interpreter in ML.

  После этого, используя те же средства и методы мы создадим простое
...
\frame{
  \frametitle{Выражения в Прологе}
%  \frametitle{Prolog terms}

  Данные и код в Прологе представляются с помощью единой системы
...
%  {\tt Fn(s,[])}.
  ...
  (разного числа принимаемых аргументов) как различные, даже если
...
%  \frametitle{Lexical analysis (1)}
   ...
   В Прологе идентификаторы, которые начинаются с заглавной буквы или
   подчёркивания, рассматриваются как переменные, а другие
   буквенно-цифровые идентификаторы наряду с числами, рассматриваются
   как константы.
...
%  semicolons.
  ...
  Непунктуационные символы можно объединять в строки любой длины,
...
\frame{
  \frametitle{Разбор и печать}
%  \frametitle{Parsing and printing}

  Принтер остаётся в точности таким же каким был раньше.
%  The printer is exactly the same as before.

  Разборщик почти такой как раньше; повторим что мы допускаем запись
  некоторых операторов, таких как сложение, в инфиксной форме.  Мы
  также допускаем специальную запись для списков.
...
%  addition to be written infix. We also allow a special syntax for lists.
  ...
  Мы рассматриваем {\tt [x1,x2,\ldots,xn]} как короткую запись для
  {\tt cons(x1,cons(x2,\ldots,cons(xn,nil)))}
%  We regard {\tt [x1,x2,\ldots,xn]} as shorthand for
%  {\tt cons(x1,cons(x2,\ldots,cons(xn,nil)))}

  где {\tt cons}~-- это функция для создания списков, подобно {\tt ::}
  в ML.  Принтер печатает её как инфиксную точку.  Также допустимо
  использовать {\tt [H|T]} вместо {\tt cons(H,T)}.
...
%  Also, {\tt [H|T]} is allowed instead of {\tt cons(H,T)}.
  ...
  {\em Правило} в Прологе это одна из следующих форм:
%  A Prolog {\em rule} is of one of these forms:
  \begin{eqnarray*}
    term\mbox{.}    & &                                             \\
    term            & \mbox{ :- } & term,\ldots,term\mbox{.}
  \end{eqnarray*}
  \noindent Для них у нас есть разборщик, который возвращает {\tt term * term list}.
...
%  \frametitle{Unification}
  ...
  Пролог использует набор правил для разрешения текущей {\em цели},
  пытаясь сопоставить одно из правил с целью.
%  Prolog uses a set of rules to solve a current {\em goal} by trying to match one
%  of the rules against the goal.

  Правило, состоящие из единственного терма, можно сразу разрешить цель.
%  A rule consisting of a single term can solve a goal immediately.

  В случае правила вида \alert{$term \mbox{ :- }
    term_1,\ldots,term_n\mbox{.}$}, то чтобы разрешить цель,
  сопоставимую с термом \alert{$term$}, Прологу необходимо разрешить
  каждый терм \alert{$term_i$} как подцель.
...
%    $term_i$} as a subgoal in order to finish the original goal.
  ...
  Однако, цели и правила могут не совпадать в точности.  Чтобы они
  совпадали, Пролог находит набор переменных.  Этот процесс называется
  {\em унификация}.
...
%    unification}.
  ...
  Это означает, что в конечном счёте мы можем получить частный случай
  изначальной цели, например получить \alert{$P(f(X))$} вместо
  \alert{$P(Y)$}.
...
\frame{
  \frametitle{Унификация: примеры и алгоритм}

  Чтобы унифицировать выражения \alert{$f(g(X),Y)$} и
  \alert{$f(g(a),X)$} необходимо задать \alert{$X = a$} и \alert{$Y =
    a$}.  Тогда оба терма будут иметь вид \alert{$f(g(a),a)$}.
%  To unify \alert{$f(g(X),Y)$} and \alert{$f(g(a),X)$} we need to set
%\alert{$X = a$} and \alert{$Y = a$}. Then both terms are
%\alert{$f(g(a),a)$}.

  Для унификации \alert{$f(a,X,Y)$} и \alert{$f(X,a,Z)$} необходимо
  задать \alert{$X = a$} и \alert{$Y = Z$}, тогда оба терма будут
  равны \alert{$f(a,a,Z)$}.
%  To unify \alert{$f(a,X,Y)$} and \alert{$f(X,a,Z)$} we need to set
%\alert{$X = a$} and \alert{$Y = Z$}, and then both terms are
%\alert{$f(a,a,Z)$}.

  Невозможно унифицировать \alert{$f(X)$} и \alert{$X$}.
%  It is impossible to unify \alert{$f(X)$} and \alert{$X$}.

  Есть общий способ отыскания {\em наиболее общего} унификатора.
%  There is a systematic procedure for finding a {\em most general}
%unifier.

  Грубо говоря, одновременно анализируем оба терма методом
  рекурсивного спуска и, найдя переменную в одном из термов, задаём ей
  значение, которое осталось в другом терме.
...
%the other side is.
  ...
  Необходимо проверять, что переменная не была задана другим значением
  и что она не используется в терме, которым она задана.
...
\frame[shrink=10]{
  \frametitle{Унификация: код}

  Мы проверяем каждую переменную была ли она задана, согласно набору
  уже заданных переменных.
...
%  proceeding.
  ...
  Мы используем набор уже заданных переменных как аккумулятор.
%  We use the existing instantiations as an accumulator.
}


\frame[shrink=10]{
  \frametitle{Пополняем набор заданных переменных}
...
\frame[shrink=10]{
  \frametitle{Поиск с возвратом}

  Пролог ведёт поиск в глубину, но может применить процедуру возврата
  (бектрекинг): даже если одно правило унифицируется, а все остальные
  цели не достигаются согласно полученному набору переменных, то
  рассматривается другое начальное правило.  Таким образом мы
  рассматриваем все цели сразу, а не только одну.

% #ISSUE: поправить текстовку, проверить семантику --lg
%         Есть словарь по ИИ:
...
\frame[shrink=10]{
  \frametitle{Другие детали}

  Заметим, что для каждого правила, мы создаём новый набор переменных.
...
%  Finally, we package everything up:
  ...
  Это означает, что либо цель не может быть разрешена, либо она
  разрешима при заданном наборе переменных.  Заметим, что мы
  возвращаем только один набор переменных, в случае если цель
  разрешима, но это легко исправить.
...
\frame[shrink=10]{
  \frametitle{Примеры на Прологе (1)}

  \PrologExamplesO
}

\frame{
  \frametitle{Примеры на Прологе (2)}

  \PrologExamplesOO

  Пролог менее <<управляемый>> чем ML, поэтому у него есть свои
  ограничения, например, следующая конструкция зациклится.
...
\frame{
  \frametitle{Доказательство теорем в стиле Пролог}
%  \frametitle{Prolog-style theorem proving}

  Метод, используемый в поиске Пролога, можно приспособить для
  создания средств доказательства теорем, внеся в него небольшие
  изменения.  Примеры таких стредств доказательства включают PTTP
  (Стикель, 1988) и \mbox{{\sf lean}$T^{\!\!\textstyle A}\!\!P$}
  (Beckert and Possega 1994).

% #ISSUE: Возможноц необходимо разместить ссылку на
...
%  (Beckert and Possega 1994).
  ...
  Унификация является рациональным методом для отыскания оптимального
  способа использования переменных, снабжённых квантором общности
  (Правиц, Робинсон 1960).
% #ISSUE: надо проверить и уточнить семантику этого предложения --lg
...
\frame{
  \frametitle{Предварительная обработка формул}
%  \frametitle{Preprocessing formulas}
  ...
  , приведа её в так называемую <<отрицательную нормальную форму>>
  (ОНФ).
...
%  \frametitle{NNF code}
  ...
  Мы определим две взаимно рекурсивные функции, которые создают ОНФ
  для формулы и для её отрицания.
%  We define two mutually recursive functions that create NNF for a
%  formula, and its negation.

  \NnfCode

  Мы преобразуем отрицание заданной формулы в отрицательную нормальую
  форму, а основное средство доказательства попробует получить из
  этого противоречие.
...
\frame{
  \frametitle{Основное средство доказательства}
...
\frame{
  \frametitle{Средство доказательства: код}
...
\frame{
  \frametitle{Средство доказательства: примеры (1)}
...
\frame{
  \frametitle{Средство доказательства: примеры (2)}
