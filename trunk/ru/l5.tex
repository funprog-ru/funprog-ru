...
\heading{Проблема с нормальным порядком редукции}
...
ML уже установлен на сервер Thor.  Чтобы использовать его, поместите
каталог с ним в пути поиска:
\begin{black}
\begin{verbatim}
...
\end{verbatim}
\end{black}
Вы можете поместить эти строки в конец вашего файла {\black \tt
  .bash\_profile}, и они будут выполняться автоматически при входе.
...
Поскольку ML является функциональныи языком, то выражения могут иметь
функциональный тип.  Синтаксис ML для обозначения $\lambda$-абстракции
{\red $\lamb{x} t[x]$} выглядит как {\black \tt fun x -> t[x]}.
Например мы можем определить функцию инкремента:
...
и показан.  Однако сама функция не была показана; система просто
...
Когда связываются функции, то можно использовать более элегантную обёртку:
...
уберите {\black \tt in} и завершите выражение:
...
Мы можем определять полиморфические функции, такие как оператор
тождества:
...
Заметим, что система сама следит за типами.  Вспомним, что {\red $I
