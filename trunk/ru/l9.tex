...
\newpagestyle{ColourDemo}%
  {\cyanonblack{Введение в Функциональное Программирование:
                Лекция 9}\hfil\cyanonblack{\thepage}}
  {\cyanonblack{Джон Харрисон}\hfil
   \cyanonblack{Университет Кембриджа, 4 февраля 1997}}
%\newpagestyle{ColourDemo}%
...
{\green Лекция 9}\\
{\green Примеры на ML I:}\\
{\green Символьное Дифференцирование}
...
\heading{Символьные вычисления}

\vspace*{0.5cm}

Речь о приложениях в которых манипуляции над математическими {\em выражениями },
в общем случае содержащими переменные
...
Axiom, Maple and Mathematica, которые могут выполнять определённые символические операции, полезные в математических расчётах.

%% There are several successful `computer algebra systems' such as
%%Axiom, Maple and Mathematica, which can do certain symbolic operations that %%are useful in mathematics.

Примеры включают разложение полиномов, дифференцирование и интегрирование выражений.

%%Examples include factorizing polynomials and differentiating and integrating
%%expressions.

Мы покажем как ML может использоваться для таких приложений. В качестве примера рассмотрим символьное дифференцирование.
...
Например, выражение {\red $sin(x + y) / cos(x - exp(y)) - ln(1 + x)$} представляется:

%%For example the expression {\red $sin(x + y) / cos(x - exp(y)) - ln(1 + x)$} %%is represented by:
...
\heading{Дифференцирование: алгоритм}

\vspace*{0.1cm}

Есть хорошо известные правила (изучаемые в школе) дифференцирования сложных выражений.
...
Они очень легко переводятся в рекурсивные алгоритмы.
...
\heading{Примеры дифференцирования}
