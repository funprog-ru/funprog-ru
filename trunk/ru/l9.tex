...
\frame{
  ...
  Списки такого вида, связывающие данные с ключами, называются {\em 
  ...
  примеров. Использование хеширования больше подойдёт для неучебных приложений.
...
\frame{
  ...
  Такое решение не является чисто функциональным, но, возможно, более естественно.
...
\frame{
  ...
  Мы будем считать оператор инфиксным, только если он встречается в списке infixes
  с приоритетом. Мы можем сделать:
  ...
  \FindInfixCodeOne
   ...
   \noindent потому что {\tt get\_precedence} не определена на неинфиксных 
   операторах. Альтернативный код использует функцию {\tt can},
...
\frame{
  ...
  The printer consists of two mutually recursive functions.
   ...
   The function {\tt string\_of\_term} takes two arguments. The first is a
   `currently active precedence', and the second is the term.

  For example, in printing the right-hand argument of {\tt x * (y + z)},
  the currently active precedence is the precedence of {\tt *}. If the
  function prints an application of an infix operator (here {\tt +}), it
  ...
  functions of the form \alert{ $f(x_1,\ldots,x_n)$}.
...
\frame{
  ...
  \item Если выражение задано в форме \alert{$f(x) * g(x)$}, применяется правило
