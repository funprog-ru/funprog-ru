...
\section*{Лекция 7. Доказательство корректности программы}
...
\frame{
  ...
  компаниями (например дефект Pentium FDIV).
...
\frame{
  ...
  Тестирование программ может служить доказательством наличия
  дефектов, но лишь в немногих случаях оно может служить
  доказательством отсутствия дефектов.
  ...
  Рассмотрим обычную математическую теорему: \alert{$$
    \Sigma_{n=0}^{n=N} n = \frac{N (N + 1)}{2} $$} Мы можем {\em
    проверить} её для множества определённых значений $N$, но легче и
  приятнее {\em доказать} её (по индукции).
...
\frame{
  ...
  того, что программа завершается.
...
\frame{
  \frametitle{Возведение в степень (1)}
  ...
  \alert{Для всех $n \geq 0$ и $x$, $\mbox{exp}\; x\; n$ завершается и
    $\mbox{exp}\; x\; n = x^n$}
...
\frame{
  \frametitle{Возведение в степень (2)}
  % \heading{Exponentiation (2)}

  \begin{itemize}
  \item Если \alert{$n = 0$}, то по определению \alert{$\mbox{exp}\;
      x\; n = 1$}.  Так как для всех чисел \alert{$x$}, мы имеем
    \alert{$x^0 = 1$}, то есть желаемый факт установлен.
...
\frame{
  \frametitle{Наибольший общий делитель (1)}
  ...
  \alert{Для любых $x$ и $y$, $\mbox{gcd}\; x\; y$ завершается и
    возвращает НОД($x$, $y$).}
...
\frame{
  \frametitle{Наибольший общий делитель (2)}
...
\frame{
  \frametitle{Наибольший общий делитель (3)}
  ...
  используем {\em обоснованную} индукцию по \alert{$|y|$}.
  % We do not, therefore, proceed by step-by-step mathematical induction, but by
  % {\em wellfounded} induction on \alert{$|y|$}.

  Идея в том, что эта величина (часто называемая {\em мера})
  уменьшается с каждым вызовом.  Мы можем использовать это свойство,
  чтобы доказать что вызов функции завершается, а также как опору для
  обоснованной индукции.
...
\frame{
  \frametitle{Наибольший общий делитель (4)}
  % \heading{Greatest common divisor (4)}

  Теперь мы представим доказательство.  Возьмём произвольное \alert{
    $n$}.  Мы предполагаем что теорема верна для всех аргументов
  \alert{ $x$} и \alert{$y$}, таких что \alert{$|y| < n$}, и мы
  попробуем доказать её для всех \alert{$x$} и \alert{$y$}, таких что
  \alert{$|y| = n$}.  Здесь два случая.
...
\frame{
  \frametitle{Наибольший общий делитель (5)}
  % \heading{Greatest common divisor (5)}

  Теперь предположим, что \alert{$y \not= 0$}.  Мы хотим применить
  предположение индукции к \alert{$\mbox{gcd}\; y\; (x\; \mbox{mod}\; y)$}.
  ...
  Поскольку \alert{$|r| < |y|$}, то согласно предположению индукции
  ...
  Нам осталось лишь показать, что он является НОД для \alert{$x$} и
  \alert{ $y$}.  Конечно же он общий делитель ибо при \alert{$d | y$}
  и \alert{ $d | r$} имеем \alert{$d | x$} так как \alert{$x = q y +
    r$}.
  ...
  в соответствии с предположением индукции получаем \alert{$d' | d$}
  что и требовалось.
...
\frame{
  ...
  \alert{$l_1$}, \alert{$l_2$} и \alert{$l_3$} имеем свойство:
  \alert{$$ \mbox{append}\; l_1\; (\mbox{append}\; l_2\; l_3) =
    \mbox{append}\; (\mbox{append}\; l_1\; l_2)\; l_3 $$} Мы можем
  использовать индукцию по длине \alert{$l_1$}, но так как функция
  была задана с помощью структурной рекурсии над списками, то
  естественнее доказывать теорему используя {\em структурную
    индукцию}.
