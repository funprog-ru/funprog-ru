...
В этой главе мы закрепим предыдущие примеры, уточним
синтаксис и основные возможности ML, а затем рассмотрим
некоторые дополнительные возможности, такие как рекурсивные типы.
Удачным началом обсуждения может стать вопрос взаимодействия с системой.
...
До сих пор мы вводили команды в интерпретатор одну за другой, получая от него
поочередно результат каждой из них. Однако, это не лучший способ писать 
нетривиальные программы. Общепринятой практикой является размещение всех 
выражений и объявлений в файле. Чтобы тестировать их по мере ввода, можно 
воспользоваться операцией <<вырезать и вставить>>, которая доступна в 
X Window System и её аналогах, или в редакторе типа Emacs.
Такой подход, однако, с ростом объема программ становится всё утомительнее 
и неэффективнее. Вместо этого возможно использовать функцию 
{\tt include} для непосредственного чтения из файла.  Например, если файл
...
\noindent то введя в интерпретатор {\tt include "myprog.ml";;} получаем:
...
содержимое файла с консоли. Последняя строка вывода~-- это результат 
...
Большие программы часто полезно снабжать комментариями. В ML они
ограничиваются символами {\tt (*} и {\tt *)}, например:
...
{\verb+->+} и конструктор декартова произведения типов {\verb+*+}.
Впоследствии мы рассмотрим и другие конструкторы, а также узнаем как
...
\item Тип {\tt unit}, который также можно называть процедурным. Множество
  допустимых значений этого типа содержит единственный элемент, который обозначается
  {\tt ()}.  Очевидно, использование типа {\tt unit} не передаёт
  какой-либо информации, так что он часто используется как
  возвращаемый тип императивных <<функций>>, таких как {\tt include}, 
  результат которых~--- не вычисленное значение, а какой-либо побочный эффект.
  Он также может использоваться
  как тип функции, используемой для приостановки вычислений.

  %% BUG# It is also a convenient argument where the only use of a
  %% function type is to delay evaluation. ? трудности перевода ?
  %% dimiii .. lg FIX#: мы знаем что ML не вычисляет под
  %% lambda-абстракциями, но какой тип дать функции, которая нужна
  %% лишь для lambda-абстракции? конечно unit
...
\item Тип {\tt int}. Содержит конечное подмножество отрицательных и
 ...
 тип целых с неограниченной разрядностью.} Элементы записываются
...
\item Тип {\tt string} содержит строки (т.е. конечные
 ...
 двойных кавычках, вот так: {\verb+"hello"+}. Для записи
 специальных символов используется экранирование в стиле языка С.
 Например, {\verb+\"+} --- двойная кавычка,
...
Значения, такие как {\tt ()}, {\tt false}, {\tt 7} и
 {\verb+"caml"+},  с точки зрения лямбда-исчисления считаются 
 константами. Другие константы соответствуют {\em операциям} над
 основными типами. Некоторые из них традиционно записываются
 в инфиксной форме. Для операций определено понятие
 приоритета, поэтому выражения группируются ожидаемым образом.
 Например, мы пишем {\tt x + y} вместо {\tt + x y} и
 {\tt x < 2 * y + z} вместо {\tt < x (+ (* 2 y) z)}. Логический
 оператор {\tt not} отличается особым правилом разбора,
 в силу чего не обладает обычным свойством левой ассоциативности:
 {\tt not not p} означает {\tt not (not p)}. Функцию, определённую
 пользователем, можно задать как инфиксную с помощью директивы
 {\tt \#infix}.  Например, вот определение функции, выполняющей
 композицию функций:
...
В ML отсутствует возможность задавать приоритеты пользовательских инфиксных 
операций, а также менять порядок применения обычных пользовательских функций
на правоассоциативный. Отметим, что неявная
 операция <<применение функции>> имеет приоритет
 выше, чем у любой другой бинарной операции, так что {\tt successor 1 * 2}
 ...
 её имя словом~{\tt prefix}:
 %% BUG# что ещё за статус??? Проверить. --dimiii .. lg FIX#: функции
 %% для которых вводится специфичный разбор, типа + или not

 %% OCAML# в Ocaml вместо `prefix'  просто заключают функцию в круглые
 %% скобки.
...
Разобравшись с этими деталями синтаксиса, рассмотрим список операций
над основными типами. Унарные операции:
...
Например, {\tt x > 0 \& x < 1} соответствует {\tt \& (> x 0) (< x 1)}.
Заметим, что не только равенство, но и все прочие отношения полиморфны. Они 
упорядочивают должным образом не только числа и строки, но также и 
остальные примитивные и составные типы. Однако, они в общем случае 
не применимы к функциям.
...
связанный в данном месте программы, рассматривается как 
переменная. Объявления связывают значения выражений с именами, при этом
...
выразить это следующей БНФ - грамматикой.\footnote{Мы не включили в неё многие 
конструкции, которые не будут использоваться в данном пособии. Подробное
определение языка приводится в руководстве по CAML.}
...
expression    & ::= & variable       \\
              ...
              & |   & \mbox{fun} \; pattern \;  \rightarrow \; expression \\
              & |   & ( expression )                                            \\
...
Позднее мы уточним определение и подробнее обсудим синтаксический 
класс~$pattern$, а на данный момент нам будет достаточно лишь конструкций
вида~$variable$ или $variable \; variable \cdots variable$. 
...
Аргументы функции записываются вслед за её именем, слева от знака равенства.
Например, объявление функции~{\tt add4}, которая  может использоваться 
для прибавления 4 к своему аргументу, имеет вид:
...
Полезно будет в качестве упражнения провести разбор этого объявления
в соответствии с приведённой выше грамматикой, к которой для определённости
требуется добавить начальное правило (аксиому). Будем считать, что 
последовательность символов, завершающаяся двойной точкой с запятой, может
быть как выражением, так и объявлением.
%
...
Несложно задать рекурсивную функцию, принимающую в качестве аргументов целое число~$n$
и функцию~$f$, а возвращающую~$f^n$, т.е. $f \circ \cdots \circ f$ ($n$~раз):
...
Нетрудно убедиться в том, что функция~{\tt funpow} принимает машинное целое~$n$ 
и возвращает нумерал (число) Чёрча, который его представляет. 
Поскольку функции не выводятся на печать, мы не сможем увидеть, как на самом 
деле выглядят лямбда-выражения, соответствующие числам Чёрча:
...
Так же просто задаётся функция, обратная к {\tt funpow}, которая принимает число
Чёрча и возвращает машинное целое:
...
\noindent Проверим некоторые из арифметических операций на числах Чёрча:
...
Рассмотрим в качестве очередного примера операцию возведения в степень.
Она отсутствует в ML по умолчанию, но её просто определить рекурсивно:
%
...
Эта реализация требует для вычисления~$x^n$ выполнения $n$~операций умножения. 
Более эффективный алгоритм использует тот факт, 
что~$x^{2 n} = (x^n)^2$ и $x^{2 n + 1} = x (x^n)^2$:
%
...
Другая классическая операция над натуральными числами~--- поиск
наибольшего  общего делителя при помощи алгоритма Евклида:
...
Мы применяли воображаемый оператор рекурсии {\tt Rec} при объяснении 
рекурсивных определений. Его можно реализовать и на практике:
%
...
Обратите внимание на необходимость анонимной функции, в противном случае выражение
{\tt Rec f} уходит в бесконечную последовательность рекурсивных вызовов, прежде
чем функция будет применена к своему аргументу:
...
Мы уже говорили о том, что ML имеет возможность объявления новых
конструкторов типов, так что составные типы могут быть построены
на базе существующих.  В действительности, ML позволяет еще больше: 
определение составного типа может также включать сам этот тип.
Подобные типы называются {\em рекурсивными}.  Они объявляются с помощью
ключевого слова {\tt type}, за которым следует выражение, определяющее тип
на основе уже заданных типов, включая его самого.
Продемонстрируем применение этой конструкции на нескольких
примерах.  Первым из них будет определение типа {\em sum}, которые
...
говоря более формально, все эти объекты имеют разные типы.
Объявление типа также определяет так называемые {\em конструкторы}
{\verb+inl+} и {\verb+inr+}.  Это функции, реализующие инъективные отображения 
множеств значений типов компонентов во множество значений нового типа. 
В этом нетрудно убедиться на практике, рассмотрев тип этих функций, выведенный
ML-системой, и результаты их применения к различным объектам: 
%
...
Мы можем продемонстрировать данную ситуацию с помощью следующей диаграммы. 
Тип~$(\alpha,\beta)sum$ создаётся на основе существующих типов $\alpha$ 
и~$\beta$, а принадлежащие этому типу значения представляют собой ни что
иное, как пару значений типа~$\alpha$ и~$\beta$ соответственно. Конструкторы 
типа~$sum$ отображают в него значения исходных типов, помещая их в нужный
элемент пары.
%
...
Этот тип данных аналогичен конструкции~{\tt union} языка~C, но в ML компоненты
различных типов хранятся отдельно, причём всегда можно точно установить, какой
именно из них определяет тип конкретного экземпляра объединения.
В противоположность этому, в языке~C элементы объединения перекрываются 
в памяти, а обеспечение корректной работы с ними возлагается всецело на 
программиста.
%
...
Приведённое определение конструкторов наделяет их тремя очень важными 
качествами:
%
...
\item 
Множество конструкторов является исчерпывающим, т.е.\ каждый элемент 
нового типа может быть получен либо из некоторого~{\tt x} при помощи 
конструктора~{\tt inl x}, либо из некоторого~{\tt y} при помощи~{\tt inr y}.
Таким образом, экземпляр нового типа не содержит ничего, кроме экземпляров 
типов компонент.
...
\item Они являются инъективными, т.е. равенство {\tt inl x = inl y} справедливо
  если, и только если {\tt x = y} (то же свойство имеет место и для {\tt inr}). 
  Таким образом, экземпляр нового типа содержит точные копии экземпляров типа 
  каждого компонента, не нарушая определённого для них понятия эквивалентности.
...
\item Они индивидуальны, т.е. их диапазоны несвязны.  Это означает, что в 
  предыдущем примере равенство {\tt inl x = inr y} будет ложным независимо от 
  того, чем являются {\tt x} и~{\tt y}. Таким образом, компоненты различных 
  типов в составе нового типа существуют отдельно друг от друга.
...
Второе и третье свойство конструкторов лежат в основе техники 
{\em сопоставления с образцом}. Аргументы $\lambda$-выражений при этом имеют
более сложный вид, соответствующий их структуре, например:
%%ISSUE:  varstruct
%
...
\noindent Эта функция имеет вполне ожидаемое свойство: когда она применяется к
{\tt inl n} она возвращает {\verb+n > 6+}, а когда она применяется к {\tt inr b}, то она
возвращает {\tt b}. Именно благодаря второму и третьему свойствам конструкторов
такое определение функции будет корректным. Поскольку конструкторы являются
инъективными, мы можем точно восстановить {\tt n} из {\tt inl n} и {\tt b} из {\tt inr b},
а их индивидуальность обеспечивает непротиворечивость выбора из двух альтернатив,
поскольку никакое значение не может соответствовать обоим образцам одновременно.
...
Наконец, исчерпывающее множество конструкторов гарантирует, что каждое значение
аргумента соответствует одному из образцов, а функция будет всюду определённой.
На практике разрешается ослабить это свойство, опустив некоторые образцы, 
но при этом ML-система выдаст предупреждение:
...
Если эта функция применяется к значению вида {\tt inl x}, то она не будет работать:
...
Хотя логический тип уже доступен в ML по умолчанию, его можно с тем же успехом 
определить при помощи достаточно простого рекурсивного типа, часто называемого 
{\em перечислимым типом}, конструкторы которого не имеют аргументов:
...
Однако, сопоставление с образцом, которое оказывается очень удобным средством 
определения действий над вариантами рекурсивных типов, этим не ограничивается.
Например, мы можем задать функцию, которая сообщает нам, является ли число 
нулём, следующим образом:
...
В этом случае мы не имеем взаимной исключительности образцов, поскольку {\tt 0}
соответствует им обоим. Образцы проверяются по порядку, один за другим, и
используется первый подходящий.  Заметим, что пока соответствие не является взаимно
исключающим, нет гарантии, что каждое выражение содержит математическое уравнение.
Так, в предыдущем примере функция не возвращает {\tt false} для любого {\tt n},
поэтому второе выражение не всюду корректно.
...
Отметим, что только {\em конструкторы} могут использоваться в приведённых выше выражениях
как компоненты образцов.  Обычные константы будут рассматриваться как новые переменные,
связанные внутри образца.  Например, рассмотрим следующий код:
...
В общем, элемент типа~{\tt unit} {\tt ()}, логические значения, целые числа, строковые
константы и операция образования пар (infix comma) имеют статус конструкторов, также как и
другие конструкторы из предопределённых рекурсивных типов.  Когда они встречаются в
образце, сопоставляемое значение должно иметь соответствующий тип.  Все другие
идентификаторы соответствуют любому выражению и становятся связанными в процессе
обработки.
...
Помимо указания образцов в $\lambda$-выражениях, существуют другие способы выполнения
сопоставления с ними.  Вместо определения функции, включающего сопоставление, и
применения её к выражению, мы можем выполнять сопоставление с образцом напрямую, используя
...
\noindent В простейшем случае достаточно ограничиться

$$ \mbox{let}\; pattern \mbox{ = } expression $$

\noindent но в этом случае разрешено использование лишь одного образца.

\subsection{Рекурсивные типы}

Предыдущие примеры рекурсивных типов не слишком оправдывали своё название, поскольку
их определения не ссылались на себя. В качестве более интересного примера рассмотрим тип
списков  \footnote{конечных упорядоченных последовательностей} элементов типа {\tt 'a}.
...
\noindent Проверим типы конструкторов:
...
Конструктор {\tt Nil}, не принимающий никаких аргументов, просто создаёт некоторый
объект типа {\verb+('a)list+}, который рассматривается как пустой список. Другой
конструктор, {\tt Cons}, принимает аргументы типа {\tt 'a} и {\tt ('a)list}, а 
возвращает новый список, который образован из предыдущего добавлением в начало
...
Поскольку конструкторы по определению инъективны и индивидуальны, легко понять, что 
все значения, которые мы рассматривали как списки $[]$, $[1]$, $[1;2]$ и $[1;2;3]$,
различны. Также из этих свойств конструкторов следует, что списки произвольной 
длины могут быть представлены значениями нашего типа. Фактически, ML уже имеет 
тип {\tt list}, точно такой, как только что был определён. 
Единственная разница в синтаксисе: пустой список обозначается {\verb+[]+}, 
а рекурсивный конструктор {\verb+::+}~-- инфиксный.
Следовательно, все упомянутые списки в действительности записываются:
...
Списки выводятся в удобном виде, это же справедливо и для ввода. Тем не менее,
следует помнить, что это синтаксический сахар, а списки можно выражать и явно,
в терминах конструкторов. Например, мы можем, используя сопоставление с образцом, 
определить функции, вычисляющие голову и хвост списка:
...
Обратите внимание, следующее определение {\tt hd} не является корректным.
В действительности, функция принимает в качестве аргумента список ровно из
двух аргументов, а на списках иной длины не определена:
...
Сопоставление с образцом может комбинироваться с рекурсией. Например, функция,
возвращающая длину списка, выглядит так:
...
В качестве альтернативы можно задать её с помощью функций {\tt hd} и {\tt tl}:
...
Последний стиль определения функции весьма распространён во многих языках, особенно
в LISP, но применение сопоставления с образцом часто бывает более элегантно.
...
Нередко полезно наглядное представление значений рекурсивных типов в виде деревьев 
с узлами, образованными применением рекурсивных конструкторов, и с листьями, в
которых хранятся значения других типов. Рекурсия в данном случае отображает
принцип построения дерева: агрегацией поддеревьев.
В случае списков <<деревья>> оказываются весьма глубокими и односторонними. 
Так, список {\tt [1;2;3;4]} представляется в следующем виде:
...
Рекурсивные типы, допускающие построение более сбалансированных 
деревьев, задаются также просто, например:
...
В общем, может быть доступно несколько различных рекурсивных конструкторов с
различным числом потомков. Это даёт очень простой способ представления
{\em синтаксических деревьев} языков программирования (и других
формальных языков). Например, следующий тип представляет арифметические
выражения, построенные из целых чисел при помощи операций сложения и умножения.
...
\noindent А рекурсивная функция вычисления значения таких выражений имеет вид:
...
Подобные абстрактные синтаксические деревья могут быть весьма удобным 
представлением данных, допускающим всевозможные манипуляции над ними. 
Так, часто первое действие компиляторов заключается в переводе, согласно 
правилам синтаксического разбора, текста на входе в <<абстрактное 
синтаксическое дерево>>. Обратим внимание, что после этого условности, вроде 
приоритета или использования скобок, уже не требуются. Как только мы 
достигаем уровня абстрактного синтаксиса, всё выражается явно за счёт 
структуры дерева. Мы будем использовать подобную технику при написании 
ML-версий формальных правил лямбда-исчисления. Начнём с типа, используемого 
для представления лямбда-термов.
...
Отметим, что вместо двух термов {\tt Abs} принимает имя переменной и терм.
Это сделано для запрета некорректных термов. В качестве примера представим 
терм $\lamb{x\; y} y\; (x\; x\; y)$ с в виде:
...
Рекурсивная функция~{\tt free\_in}, определяющая, является ли
переменная в терме свободной, имеет вид:
...
конфликта имён, нам потребуется функция для переименования переменных. Мы хотим
преобразовывать существующие имена переменных в новые, не свободные в 
заданном выражении. Переименование производится путём добавления 
апострофа~(\texttt{'}) в конец имени.
...
\noindent Теперь мы можем определить подстановку так, как мы это сделали
для абстрактных термов:
...
Обратим внимание на очень близкие параллели между стандартным 
математическим представлением лямбда-исчисления, которое обсуждалось
ранее, и ML-версией. Всё что нам действительно надо для завершения
аналогии, это  вместо явного вызова конструкторов читать и записывать
термы в более удобной форме. Мы вернёмся к этой проблеме позже и обсудим, 
как можно добавить такие возможности.
...
\subsection{Тонкости рекурсивных типов}

Рекурсивный тип может содержать вложенные конструкторы других типов,
включая конструктор типа функций. Например, рассмотрим следующее определение:
...
Если мы задумаемся о его семантике, то столкнёмся с определёнными сложностями.
Рассмотрим, к примеру, специальный случай, когда {\verb+'a+} является 
{\verb+bool+}. Конструктор {\verb+K:((bool)embedding->bool)->(bool)embedding+} 
по определению считается инъективной функцией.
Это прямо противоречит теореме Кантора о том, что множество всех подмножеств 
множества~$X$ не может  быть инъективно ему.\footnote{Доказательство: 
рассмотрим $C = \{ i(s) \mid s \in \powerset(X) \mbox{ и } i(s) \not\in s \}$. 
Если $i:\powerset(X) \to X$ инъективно, то справедливо $i(C) \in C
\Iff i(C) \not\in C$, получили противоречие. Эта конструкция напоминает парадокс Рассела,
и, возможно, в действительности вдохновлена им. Аналогия будет ещё более близкой,
если мы рассмотрим равнозначное утверждение о невозможности сюръективного отображения $j:X \to \powerset(X)$, 
и докажем его, рассматривая $\{s \mid s \not\in j(s)\}$.} Следовательно,  
рассуждая о семантике типов, нам следует быть осторожнее. В действительности,
$\alpha \to \beta$  не может рассматриваться как полное функциональное 
пространство, иначе конструкции рекурсивных типов, подобные приведённой выше, будут противоречивыми.
Поскольку все функции, которые мы можем задать, в действительности являются
вычислимыми, есть смысл ограничиться только ими. С таким ограничением возможно
построение непротиворечивой, хотя и сложной, семантики.
...
Рассмотренное определение также имеет интересные последствия для системы типов.
Например, мы можем теперь определить $Y$~комбинатор, используя~$K$ для 
приведения типа. Отметим, что удалив все вхождения~$K$, мы получим, в сущности, обычное
определение $Y$~комбинатора в нетипизированном лямбда-исчислении. Конструкция~{\tt let}
 используется исключительно для эффективности, но мы  {\em нуждаемся} 
в $\eta$-редексе, включающем {\tt z}, чтобы предупредить зацикливание, имеющее
место в стратегии вычислений, принятой в ML.
...
Таким образом, рекурсивные типы представляют собой значительный вклад в 
язык и позволяют обойтись без введения в него оператора рекурсии как отдельной
...
\item Что будет неверно в нашем определении {\tt subst}, если мы заменим
%\item What goes wrong in our definition of {\tt subst} if we replace
{\verb+Var y -> if x = y then t else Var y+}
сопоставлением с двумя образцами:
%\noindent by these separate patterns?
{\verb+Var x -> t | Var y -> Var y+}?

\item 
Натуральные числа возможно представить (хотя и неэффективно) при помощи
...
Определите функции, выполняющие арифметические действия над числами в 
этой форме.

\item Пользуясь представлением синтаксиса лямбда-термов, 
данным выше, напишите функцию, конвертирующую произвольный лямбда-терм
в эквивалентный, записанный с помощью $S$, $K$ и $I$~комбинаторов, при помощи
техники, рассмотренной нами ранее.
В качестве представления комбинаторов используйте {\verb+Const "S"+} 
и т.п.


\item Расширьте синтаксис лямбда-термов, включив типизацию в стиле Чёрча.
Реализуйте функции, выполняющие проверку правил типизации в ходе построения
термов и представление последних в виде композиции примитивных 
конструкторов.

\item Рассмотрим тип бинарных деревьев со строками в узлах:
%
...
Напишите функцию {\tt strings}, возвращающую список строк, полученный при обходе дерева 
в порядке слева направо, т.е.\ такую, которая для каждого узла дерева добавляет
в конец списка, полученного при обходе левого поддерева, одноэлементный список
со строкой, расположенной в данном узле, а затем --- список строк, полученный
при обходе правого поддерева, сохраняя их порядок. 
Напишите функцию, которая принимает строку и дерево, для 
которого соответствующий список отсортирован в алфавитном порядке, а возвращает
другое дерево, в которое данная строка вставлена с сохранением упорядоченности. 

\item (*) Улучшите эти функции так, чтобы на каждом шагу дерево оставалось почти 
сбалансированным, т.е. для каждого узла максимальная  глубина левого и правого 
поддерева отличалась бы не более чем на единицу.\footnote{Подробнее об 
этих АВЛ-деревьях и похожих методах построения сбалансированных деревьев 
см.\ работу~\citeN{avl} либо другие книги по алгоритмам.}

\item (*) Можете ли вы описать типы выражений, которые могут быть представлены
в типизированном лямбда-исчислении без применения оператора рекурсии?%
\footnote{Ответ на этот вопрос содержится в так называемом
<<гомоморфизме Карри-Говарда>>~--- подробнее см. 
\citeN{girard-prat}.} Каков тип следующей рекурсивной функции?
...
% ISSUE Смысл перевода ускользает. Думать.
Покажите, как это может использоваться для записи ML выражений с полностью 
полиморфным типом $\alpha$. Можете ли вы написать завершённое выражение
с таким свойством? Как насчёт произвольной функции типа $\alpha \to \beta$?
