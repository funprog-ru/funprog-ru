...
% BUG# вместо 'линия вывода' может лучше 'строка вывода'? --lg
...
\item Тип {\tt unit}, можно называть его процедурным.  Это
  ...
  Например, таких как {\tt include}.  Это также подходящий тип для
  аргумента FIX (?It is also a convenient argument where the only use
  of a function type is to delay evaluation.?)
...
\item Тип {\tt string} содержит строки (т.е. конечные
 ...
 Например, (FIX) \lstinline!"! - запись двойной кавычки,
...
как константы. Другие константы соответствуют {\em операциям} на
 ...
 {\tt \#infix}.(FIX в окамле () используется + ещё какие - то условия)
...
В ML нет возможностей задавать приоритет для инфиксных операций,
 ...
 операция 'аппликация функции'(FIX check in wiki) имеет приоритет
 выше, чем у любой бинарной операции, так что {\tt successor 1 * 2}
 разбирается как {\tt (successor 1) * 2}. Если хотите использовать
 функцию со специальным статусом (FIX что ещё за статус???) как
 обычную константу, вам потребуется {\tt prefix}:
