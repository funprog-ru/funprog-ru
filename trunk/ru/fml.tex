...
как константы. Другие константы соответствуют {\em операциям} на
 ...
 {\tt \#infix}.  Например, вот определение функции, выполняющей композицию функций:
...
В действительности, небольшое размышление показывает, что функция~{\tt funpow} принимает
машинное целое~$n$ и возвращает нумерал (число) Чёрча, соответствующее~$n$. Поскольку функции
не  выводятся на печать, мы не можем в действительности посмотреть на лямбда-выражение,
...
конструкторов, что дает нам хорошо определенную функцию.  Поскольку конструкторы являются
...
везде.  В действительности, разрешается ослабить это свойство путем неуказания некоторых
...
В самом деле, разрешается определять выражения путем сопоставления с истинным значением.
...
сообщает нам является ли число нулем, следующим образом:
...
Заметьте, что только {\em конструкторы} могут использоваться в вышеприведенных выражениях
...
другие конструктора из предопределенных рекурсивных типов.  Когда они встречаются в
...
применения ее к выражению, мы можем выполнять сопоставление с образцом напрямую используя
