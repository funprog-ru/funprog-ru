...
на языке математики. Вероятно, лучший анализ этой задачи был дан 
Тьюрингом~\cite{turing}, утверждавшим, что механическими можно считать 
...
полнотой за счёт отсутствия ограничений на объём доступной памяти. Строго 
говоря, любой существующий компьютер алгоритмически эквивалентен конечному
автомату, но для удобства принято также полагать объём его памяти бесконечным.}
...
лямбда-исчисление, первоначально предназначенное на роль формальной основы 
...
постулат получил название {\em тезиса Чёрча}. В дальнейшем было
показано, что из данного тезиса следует неразрешимость 
{\em Entscheidungsproblem}~\cite{church-number}. Тьюринг впоследствии доказал, что
...
это будет рассмотрено в отдельном курсе, посвящённом теории алгоритмов.} 
...
функций высших порядков, зато имелся существенный объём императивных 
...
областей видимости переменных в Algol~60). Ним же было
...
(<<If you See What I Mean>>---досл.\ <<Если вам понятно, о чём речь>>)~\cite{landin-700}.
...
определённого способа представления данных в виде выражений лямбда-исчисления.
...
удобства восприятия либо подчёркивания ассоциативности. На самом деле, мы можем
...
{\em нумералов Чёрча,} хотя его базовая идея была опубликована ранее  
Витгенштейном~\cite{wittgenstein-tractatus}.%
...
Нам требуется выражение~$\mbox{PRE}$ такое, что~$\mbox{PRE}\; 0 = 0$ и~$\mbox{PRE}\; (n + 1) = n$.
Оригинальное решение этой задачи было предложено Клини~\cite{kleene-numbers}.
Пусть для заданного~$\lamb{f\;x} f^n\; x$ требуется
<<отбросить>> одно из применений~$f$. В качестве первого шага введём 
на множестве пар функцию~$\mbox{PREFN}$ такую, что
%
...
не испытывая особых затруднений. Определение~$\mbox{PREFN}$, удовлетворяющее
нашим нуждам, таково:
%
...
С учётом этих соображений альтернативное определение Тьюринга может оказаться
...
лямбда-терм не имеет нормальной формы, т.~е.\ в известном смысле не определён.
...
что подробнее рассматривается в работах Барендрегта~\cite{barendregt} 
и Абрамски~\cite{abramsky-lazy}.
}
...
чистого лямбда-исчисления. Примечательно, что этих средств достаточно
...
вопросов, затронутых в этом разделе. В частности, М.~Гордоном
...
<<проблемы останова>> в лямбда-исчислении) алгоритмически неразрешима~\cite{gordon-plt}. Влияние
лямбда-исчисления на полноценные языки программирования, а также на эволюцию
функциональных языков в частности обсуждается в работе Худака~\cite{hudak-survey}. 
%
...
\item Дайте обоснование <<обобщённого $\beta\hbox{-преобразования}$>>, 
т.~е.\ докажите, что
$$
...
\item 
Какие арифметические операции соответствуют заданным на множестве нумералов 
...
\item 
Пусть задано следующее представление списков~\cite{mairson-param}:
