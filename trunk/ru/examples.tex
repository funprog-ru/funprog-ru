...
Maxima, Maple and Mathematica. Они могут выполнять полезные
...
но это даст нам  точное представление об основных достоинствах языка и
мы сможем запустить несколько примеров.
...
порядка. Мы уже определяли тип термов для наших математических выражений, их 
печати, и соответствующих парсеров. Сейчас мы будем использовать нечто
похожее, но с некоторыми модификациями. Во-первых, немного упростим код, будем 
рассматривать константы как нуль-арные функции, т.е. как функции, принимающие 
пустой список аргументов. Соответственно определим:
...
{\verb+Fn(s,[])+}. Обратите внимание, что мы будем рассматривать функции различной арности
(разным числом аргументов) как различные, даже если у них одинаковое имя.
Следовательно нет опасности, что константы будут спутаны с настоящими
функциями.
...
В целях следования соглашениям пролога, которые включают чувствительность к
регистру, мы обновим функции лексического анализа. Мы не будем требовать
точного совпадения во всём, но одна деталь очень важна: 
символьно-цифровые идентификаторы, начинающиеся с буквы {\em в верхнем регистре},
рассматриваются как переменные, а другие символьно-цифровые идентификаторы и
числа~-- как константы. Например, {\verb+X+} и {\verb+Answer+}~-- переменные,
тогда как {\verb+x+} и {\verb+john+}~-- константы. Мы будем смешивать все 
символьные идентификаторы вместе с константами, но будем различать
символы пунктуации: левую и правую скобки, запятую и точку с запятой. 
Символьные идентификаторы должны состоять более чем из одоног символа
Непунктуационные символы собираются вместе, так что  
We
...
как был ранее, система печати не меняется. Единственная модификация~-- прологовские
списки записываются в более удобной нотации. В Прологе <<.>> и <<nil>> 
соответствуют  <<{\tt ::}>> и <<{\tt []}>> в ML. Мы также допускаем 
прологовскую запись <<{\verb+[H|T]+}>> вместо <<{\verb+cons(H,T)+}>>, обычно
...
цель немедленно, если соответствует цели. В случае правила $term \mbox{ :- } 
term_1,\ldots,term_n\mbox{.}$, если цель соответствует $term$, то Прологу 
необходимо разрешить, как подцель, по порядку каждый $term_i$. Если он
может выполнить это, первоначальная цель считается разрешённой.  
...
Однако, цели и правила не обязаны в точности совпадать. Взамен Пролог назначает
переменные для цели и сопоставляемого правила, такое действие называется 
{\em унификацией}. Это означает, что мы можем прервать проверку отдельного
варианта оригинальной цели, т.е.  $P(f(X))$ вместо $P(Y)$.
Например:
...
\item Для унификации $f(g(X),Y)$ и $f(g(a),X)$ мы можем присвоить
$X = a$ и $Y = a$. После этого термы будут: $f(g(a),a)$.

\item Для унификации $f(a,X,Y)$ и $f(X,a,Z)$ мы можем присвоить 
...
В общем случае, унификаторы не уникальны. Например, во втором примере мы
могли выбрать присваивание $Y = f(b)$ и $Z = f(b)$. Однако, всегда можно
выбрать  {\em most general}, i.e. any other unification can be reached from
...
Пролог выполняет поиск в глубину, но он может использовать возврат: даже если
правило неуспешно унифицируется, если все последующие цели не могут быть разрешены
при текущих связываниях переменных, то сверяется другое исходное (первоначальное)
правило. Следовательно, мы рассматриваем целый список целей, вместо одной за
раз. 
...
Мы используем функцию {\tt rename} для генерации новых переменных:
...
Наконец, мы можем собрать всё вместе, в одну функцию {\tt prolog}, которая
пытается разрешить заданное правило, используя правила: 
...
Мы можем использовать только что написанный интерпретатор для
запуска примеров из какой-нибудь книги по Прологу. Например:
...
хрупка. Например, следующие циклы неопределённы:
