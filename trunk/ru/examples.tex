...
будет напечатана целиком, мы можем выполнить то же самое для каждой из его 
составляющих, указывая при этом системе печати возможные точки разбиения на 
строки при помощи вызова специальных функций.
Многие из принципов, использующиеся в этой и подобных системах форматированной
печати, обсуждаются в работе~\cite{oppen-pretty}. Мы не будем рассматривать их 
...
\footnote{По сути, мы будем придерживаться экстенсионального подхода и
манипулировать функциями, не интересуясь их внутренним представлением.}
...
точностью, пока мы не убедимся в том, что он отличен от нуля. Следующая лемма служит
...
%
$$ 
  f_y(n) = 2^{n+k} \mathbin{\mbox{\tt ndiv}} f_x(k),
$$
%
...
соответствие ссылку на ячейку памяти, в которой будем хранить её значение, 
