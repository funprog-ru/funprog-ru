...
Мы просто применяем упрощение, начиная с корня дерева выражения. Мы выполняем 
его в обходе снизу - вверх. 
% This will also bring negations upwards through products. 
% - что - то про знак "-"  
В некоторых случаях было бы более эффективно упрощение при обходе сверху - вниз, 
например, для $0 * t$ и сложного выражения $t$, но это потребует повторения
в случае термов вроде $(0 + 0) * 2$. Выбор здесь напоминает выбор между
нормальным и аппликативным порядком редукции в лямбда-исчислении.
...
Такая запись и короче, и более привычна. Тем не менее, такое решение основывается
на том, что мы знаем определения  довольно неприметных тригонометрических функций,
вроде {\tt sec}. Что бы машина не решила делать, вероятно, найдётся
пользователь, которого это не устроит.
...
языков. Другие символьные операции
