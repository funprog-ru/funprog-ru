...
Клину.  Теперь определим: {\red $$ \mbox{PRE}\; n \defeq \lamb{f\; x}
...
определить {\red $\mbox{fact}$} следующим образом:
...
{\red $F$}, т.е. {\red $\mbox{fact} = F\; \mbox{fact}$}.  Поэтому мы определим:
{\red $$\mbox{fact} = Y\; F$$}
