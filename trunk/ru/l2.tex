...
\frame{
  ...
  Эти правила могут быть применены машинально, без раздумий, как можно
  было бы преобразовать выражение:
  % BUG# как-то коряво звучит по моему --lg
  \alert{$$ x - 3 = 5 - x$$}
  в
  \alert{$$ 2 x = 5 + 3 $$}

  без учёта основных значений.
...
\frame{
  ...
  \begin{itemize}
  \item Если \alert{$t$} переменная \alert{$v$}, то \alert{$FV(t) = \{v\}$}
  ...
  \item Если \alert{$t$} константа \alert{$c$}, то \alert{$FV(t) =
      \emptyset$}, которое заведомо конечно.
    % \item If \alert{$t$} is a constant \alert{$c$}, then \alert{$FV(t) = \emptyset$},
    %   which is trivially finite.
    \maybepause

  \item Если \alert{$t$} применение \alert{$s_1\; s_2$}, то по
    предположению индукции \alert{$FV(s_1)$} и \alert{$FV(s_2)$} оба
...
\frame{
  ...
  Мы записываем \alert{$t[s/x]$} как результат подстановки терма \alert{
    $s$} вместо переменной \alert{$x$} в другом терме \alert{$t$}. Иногда
...
\frame{
  ...
  \alert{
    ...
    \end{eqnarray*}}
  Однако это не так.  Мы получим \alert{$(\lamb{y} x + y)[y/x] = \lamb{y} y
...
\frame{
  ...
  Для определённости мы можем определить \alert{$z$} как самую первую
...
\frame{
  ...
  которая переводит один терм в другой.  А записывают это как \alert{$s =
    t$}.
  ...
  Применив $\eta$-конверсию пару раз, и получим \alert{$f = g$}.
...
\frame{
  ...
  утверждение, чем то, что если \alert{$t_1 = t_2$}, то существует терм
...
\frame{
  ...
  Достаточно найти комбинаторный эквивалент для любого \alert{$\lamb{x}
    t$}, где \alert{$t$} терм \alert{$\lambda$}-свободный.  Тогда мы
  ...
  По сути, достаточно \alert{$S$} и \alert{$K$}, ибо \alert{$I = S\;
    K\; K$}.
