...
\item Правила преобразований и эквивалентность $\lambda$-термов
...
Мы принимаем некоторые факты, как `очевидные', т.е.
{\red $$ (\lamb{x\; y} x + y)\;1\;2 = (\lamb{y} 1 + y)\;2 = 1 + 2 $$}
потому что они соотносятся с интуитивным смыслом.  Сейчас мы зададим
...
Как пояснение к тому, как можно определять функции рекурсивно,
используя синтаксис $\lambda$-термов, вот формальное определение
...
относительно значений для подтермов, как здесь. Интуитивно ясно, что
мы всегда можем вычислить значение функции для какого-либо конкретного
терма.
...
\heading{Правила преобразований}
...
эквивалентные термы.  Конверсии традиционно обозначаются греческими
буквами: {\green $\alpha$} (alpha), {\green $\beta$} (beta) и {\green
  $\eta$} (eta).  Самая важная для нас это {\green $\beta$}-конверсия.
